...
<\body>
  ...
  <\enumerate>
    ...
    <\enumerate>
      ...
      <\enumerate>
        <item><math|n\<in\>2\<bbb-Z\>+1> case

        <item><math|n\<in\>2\<bbb-Z\>> case
      </enumerate>

      <item>Compare it with <math|q=2> case\ 

      <item>First start with <math|k=0> and <math|q=2> case done in
      <verbatim|KP-normalization.pdf> to check for inconsistincies.

      <item>Then, do the <math|q=2> and <math|k\<in\>\<bbb-Z\><rsub|\<geqslant\>0>>
      case done in <verbatim|KP-normalization.pdf> to check for
      inconsistincies.
