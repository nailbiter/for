...
<\body>
  ...
  <\enumerate>
    <item>Read pp. 1-11 of <cite|grauert2012several>
  </enumerate>
  ...
  <\enumerate>
    ...
    <math|<around*|{|y<rsub|j>|}><rsub|j=1><rsup|q>>. Suppose it is invariant
    under <math|O<around*|(|p-1,q|)>> (acting on <math|\<bbb-R\><rsup|p,q>>,
    so that it fixes <math|x<rsub|p>>). Then it can be shown that <math|g> is
    in fact polynomial in <math|x<rsub|p>> and
    <math|<wide|Q|~>\<assign\><big|sum><rsub|i=1><rsup|p-1>x<rsub|i><rsup|2>-<big|sum><rsub|j=1><rsup|>y<rsub|j><rsup|2>>''.

    <item>Decide how to divide word done in master thesis into 2 or 3
    articles.
    ...
    <\enumerate>
      ...
      ``sufficiently regular'' parameters (perhaps, it cannot be defined for
      general distributions, but we can use special properties of kernels
      (e.g. homogeneity) to make sense of it);
      ...
      when restricted to <math|Q\<gtr\>0> open subset (as Fourier transforms
      of <math|A<rsub|\<lambda\>,\<nu\>>> and
    ...
    <\enumerate>
      ...
      <item>cosets intersect transversally (cf. wavefronts) afar from origin.
    </enumerate>
  </enumerate>
  ...
  <\bibliography|bib|alpha|report.bib>
    <\bib-list|GF12>
      <bibitem*|GF12><label|bib-grauert2012several>Hans Grauert and Klaus
      Fritzsche. <newblock><with|font-shape|italic|Several complex
      variables>, volume<nbsp>38. <newblock>Springer Science & Business
      Media, 2012.
    </bib-list>
  </bibliography>
</body>
