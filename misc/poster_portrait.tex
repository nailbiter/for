...
\end{figure}
  ...
  \mybox{Definition (SBO)}{
	  Suppose $G'\xrightarrow{\;\;\pi'} GL_{\C}(W)$ another representation.
	  SBO (symmetry breaking operator) is a linear map $A:V\to W$ satisfying $\forall g'\in G'$
\begin{figure}[H]
\hspace{1.3cm}
\xymatrixrowsep{0.2pt}
\xymatrix{
...
\mybox{This work concerns:}{
	Program $\mathcal{C}$ for SBOs
	\begin{enumerate}
...
%%		\hspace{-0.25cm}$G\curvearrowright$ \raisedtikzcone=$\left\{ (x,y)\in\R^{p+1}\times\R^{q+1}\mid \myabs{x}=\myabs{y}\neq0 \right\}.$\\
%%		\vspace{-0.35cm}
%%		\begin{equation*}
%%			\kern-0.5cm\leadsto G\curvearrowright \pi:=I(\lambda):=
%%		\end{equation*}
%%		\vspace{-0.6cm}
%%		\begin{equation*}
%%			\kern-0.4cm:=\left\{ f\in C^\infty\left( \raisebox{-0.2cm}{\tikzcone{0.05}} \right) \mbox{: homogeneous of deg.} -\lambda \right\}.
%%		\end{equation*}
%%		Similarly for $\pi'=J(\nu)$ of $G'$.
 	}
 }
...
\headerbox{{R}esult \#6}{name=functional,column=2,below=residue}
{
	\begin{definition}
