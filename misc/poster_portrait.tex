...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \headerbox{Philosophy}{name=setting,column=0,row=0}{
	  \begin{tcolorbox}[colback=green!5,colframe=green!40!black,title= ``Program for branching problems''\cite{kobayashi2015program}]
		  Representation and its restriction:
\begin{figure}[H]
	\hspace{1.35cm}
...
\end{figure}
	...
	\end{tcolorbox}
	\mybox{ABC Program of Branching \cite{kobayashi2015symmetry}}{
...
We say $A:V\to W$ is {\it symmetry breaking operator} ({\it SBO} for brevity) if $\forall g'\in G'$
\begin{figure}[H]
\hspace{1.3cm}
\xymatrixrowsep{0.2pt}
\xymatrix{
...
\mybox{This work concerns:}{
	Program $\mathcal{C}$ for SBOs (symmetry breaking operators)
	\begin{enumerate}
		\item[($\mathcal{C}$1)] Construct SBOs;
		\item[($\mathcal{C}$2)] Classify SBOs;
		\item[($\mathcal{C}$3)] Functional equations;
		\item[($\mathcal{C}$4)] Residue formulae;
...
\end{tikzpicture}
 ...
 {
	 ($\mathcal{C}1$--$\mathcal{C}2$) were proposed and carried out in \cite{kobayashi2015symmetry} for  
	 rank one case $(G,G'=( O(n+1, 1), O(n,1)))$.
	 We generalize to higher rank case $(G,G')=(O(p+1,q+1, O(p,q+1)))$.\\
	 \tikzcone{0.3}
 }
...
\[ q_C^X (\lambda, \nu) : = \left\{ \begin{array}{ll}
     ...
     2\mathbbm{Z}+ 1.
   \end{array} \right. \]
   \[q_Y^X (\lambda, \nu) : = \raisebox{-0.4cm}{\mbox{\includegraphics[scale=0.1,trim=0mm 12mm 0mm 0mm]{images/qr}}}\]
}
...
\headerbox{Result \#3 (classif. of SBOs)}{name=classif,column=2,row=0}
{
...
\headerbox{Result \#4 }{name=spherical,column=2,below=classif}
{
