...
(define (signswitch . z)
	(define (inner2 l ctr) (if (null? l) '() (cons (if (even? ctr) (car l) (* -1.0 (car l))) (inner2 (cdr l) (+ ctr 1)))))
...
(define (scale f)(- (* 10 f) 5))
(define (pairs->list lp)(let* ((p1 (car lp))
                               (p2 (car (cdr lp)))
                               )
                          (list (car p1)(cdr p1)(car p2)(cdr p2))))
(define (generate-l)(list (cons (scale(random:uniform))(scale(random:uniform)))(cons 0.0 0.0)))
(define (generate-l2)(list (cons 0.0 0.0)(cons (scale(random:uniform))(scale(random:uniform)))))
(define (get-mat) (let* ((l1 (generate-l2))
                (l2 (map (lambda (p) (cons (-(cdr p))(car p))) l1))
                (l3 (generate-l))
                (l4 (map (lambda (p) (cons (-(cdr p))(car p))) l3))
                )(map pairs->list (list l1 l2 l3 l4))))
(define (trial cur end)(if (> cur end)(exit)
                         (let* ((mat (get-mat))
                                (d (det mat))
                                )
                           (if (< d 0) (begin (display mat)(newline)(display d)(newline)(display "EPIC WIN")(newline)(exit))
                             (begin(display d)(newline)(trial (inc cur) end))))))
(trial 0 10250)
