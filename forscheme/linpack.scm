...
;printing procedures
(define (print-number/latex num)(cond((integer? num)(format #t "~d" num))((rational? num)(let((num(numerator num))
    (den(denominator num)))(begin(if(> 0 num)(display "-")'())(format #t "\\mysfrac{~d}{~d}" (abs num) den))))))
...
(define (print-augmented-matrix/latex M aug-cols) (begin (format #t "\\left(\\begin{array}{")
			       (map (lambda (n) (format #t "r")) (seq 0 (- (length (car M)) aug-cols)))(format #t "|")
                   (map (lambda (n) (format #t "r")) (seq 0 aug-cols))
			       (format #t "}\n")
			       (map (lambda (row) (begin (print-number/latex (car row))
							 (map (lambda (e) (begin(display "&")(print-number/latex e))) (cdr row)) (format #t "\\\\\n"))) M)
			       (format #t "\\end{array}\\right)\n")
			       ))
...
(map (lambda(row)(begin(display row)(newline)))
      (fold (lambda (op mat) (let* ((res (cond 
                                    ...
                                    (dummy (format #t "\\["))
                                    (dummy (print-augmented-matrix/latex mat aug-col))
                                    ;(dummy (cond 
                                    ;((eq?(car op)'m) (let*((rownum(list-ref op 1)) (mult(list-ref op 2)))
                                    ;                          (format #t "\\xrightarrow{\\text\\textcircled{~d}"rownum)))
                                    ;((eq?(car op)'i)(let*((i1 (cadr op))(i2 (caddr op))(r1 (list-ref mat i1))(r2(list-ref mat i2)))
                                    ;                    (replace-elem (replace-elem mat i1 r2) i2 r1)))
                                    ;((eq?(car op)'s)(let*((i1(cadr op))(i2(caddr op))(m (cadddr op))(r1(list-ref mat i1)))
                                    ;                  (replace-elem mat i2 (map (lambda (a b)(- b(* m a) )) r1 (list-ref mat i2)))))
                                    ...
                                    ;))
                                    (dummy (print-augmented-matrix/latex mat aug-col))
                                    (dummy (print-augmented-matrix/latex res aug-col))
                                    (dummy (format #t "\\]")))
                         res
                             )) A (list (list 's 0 1 (/ 1 2))
                                        ;(list 'm 0 2)
                                        (list 'm 1 -2)
                                        ;(list 'i 1 2)
                                        )))(newline)
