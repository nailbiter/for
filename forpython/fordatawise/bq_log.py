...
def bq_log(start_date, slack, slack_webhook, bigquery_credentials):
    ...
    df = pd.concat([
        pd.DataFrame([
            ...
            in client.list_jobs(project=project, state_filter="done", min_creation_time=datetime(start_date.year, start_date.month, start_date.day))
        ])
        for project in ["olm-1-datawarehouse-dev", "olm-limited-access"]
    ])
