...
@click.option("--agg", type=click.Choice([*_GROUPBY, "all"]), multiple=True)
@click.pass_context
def show_score(ctx, deck_index, deck_size, full, sort, agg):
    ...
    if len(agg) > 0:
        ...
        _logger.info(f"columns: {list(deck_df)}")
        _all = {"deck_index", *_GROUPBY}
        _logger.info(f"_all: {_all}")
        _agg = list(_all-{*agg})
        _logger.info(f"_agg: {_agg}")

        def _aggregator(x):
            idx, df = x
            __logger = _logger.getChild("_aggregator")
#            __logger.info(f"idx: {idx}")
            __logger.info(f"columns: {list(df)}")
#            __logger.info(f"df: \n{df}")
            return pd.DataFrame([{
                **{k: v for k, v in zip(_agg, idx if len(_agg) > 1 else [idx])},
                **{k: v.mean() if k == "score" else v.sample() for k, v in df.items() if k not in _all},
            }])

        deck_df=pd.concat(map(_aggregator, deck_df.groupby(
            _agg, as_index=False))).reset_index().set_index(_agg)
