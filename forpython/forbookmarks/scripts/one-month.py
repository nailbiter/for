...
def pdf(ctx):
    ...
    if month is None:
        month = datetime.now()
    days = [
        datetime(month.year, month.month, d + 1)
...
def email(ctx):
    ...
    with open(text_fn, "w") as f:
        f.write(text)
    cmd = Template(
        """send_email.py {%for r in recipients%} -r '{{r}}' {%endfor%} -t {{text}} -s 'Закладки на {{month}}' {%for day in days%}-a pdfs/bookmark_{{day.strftime("%Y-%m-%d")}}.pdf {%endfor%}"""
    ).render(
        {
            "days": days,
            "month": _umonth,
