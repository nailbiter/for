...
def send_notification(message, media,delay_min):
    ...
    if "slack" in media:
        client = MongoClient()
        slack_webhook = client.admin.passwords.find_one({"key":"DTWS_SLACK_WEBHOOK"})["value"]
        #FIXME do not use system
        system(f"curl -X POST -H 'Content-type: application/json' --data '{{\"text\":\"{message}\"}}' \"{slack_webhook}\"")
    if "email" in media:
