...
\newcommand{\myrelationdiagram}[5]{
		\centerline{
		\xymatrix{
			\frame{\mbox{#1}}\ar@{=>}[rd]^{\begin{array}[]{c}
				#2
			\end{array}}&&\frame{\mbox{#4}}\ar@{=>}[ld]_{\begin{array}[]{c}
			#3
	\end{array}}\\
			&
			\frame{\mbox{#5}}
...
\newcommand{\myrelationdiagramBig}[5]{
		...
		\xymatrix{
			\frame{\mbox{#1}}\ar@{=>}[rd]^{\begin{array}[]{c}
				#2
			\end{array}}&&&&\frame{\mbox{#4}}\ar@{=>}[ld]_{\kern-1.5cm\begin{array}[]{c}
			#3
	\end{array}}\\
			&&&&
		}}
...
%%	\end{enumerate}
			...
			\end{lemma*}
\end{method}
\begin{method}{メソッド 4 (積分公式とルジャンドル関数$\kern-0.1cm P^\alpha_\beta\kern-0.1cm$を用いる}
