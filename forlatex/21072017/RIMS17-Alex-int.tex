...
得られた公式の特殊値や極限値と、古典的な積分公式やWarnaar、Varchenko，Tarasov積分などによる種々のSelberg型積分の特殊値との関連について説明したい．主結果の複数の証明方法を挙げる。
...
\section{主結果}
	...
	\end{equation*}
	を満たす多項式である。さらに、
	$\lambda$が実数数で$\lambda>-\frac{1}{2}$をみたすとき、
	Gegenbauer多項式$\left\{ C_n^\lambda \right\}_{n=1}^{\infty}$は
	$L^2\left( [-1,1],(1-x^2)^{\lambda-\frac{1}{2}}dx \right)$の直交多項式とな\doubt{る}。
	最初の数項を挙げよう。
	\begin{example*}
%%		最初のいくつは
		\begin{eqnarray*}
