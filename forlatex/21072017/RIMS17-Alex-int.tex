...
\section{主結果}
	...
	$\lambda$が定数で$\lambda>-\frac{1}{2}$をみたすとき\doubt{は}
	$L^2\left( [-1,1],(1-x^2)^{\lambda-\frac{1}{2}}dx \right)$の直交多項式である.
...
%%		最初のいくつは
	...
	\begin{prop}\label{prop:exp-stz-gg}
		  \label{thm:4}$\tmop{Re} \lambda, \tmop{Re} \mu > - \frac{1}{2}$,
		    $\tmop{Re} \nu > 0$ かつ $-1 \leqslant z \leqslant 1$ のとき、
		    \begin{equation*}
			    \begin{array}{c}
...
\begin{equation*}
	...
	積分公式と同値である.
	\begin{taggedprop}{\taggedpropsymb}
...
\date{} % Activate to display a given date or no date (if empty),
         % otherwise the current date is printed 嘘頼める
