...
%%		最初のいくつは
	...
	\begin{thm}\label{prop:exp-st-gg}
		...
		の有理式\footnote{I might be very wrong, but I thought that $a^{\ell,m}_{\lambda,\mu,\nu}$ and $b(\lambda,\mu,\nu)$ are \textit{not} a 
	rational expressions in $\lambda,\mu,\nu$, because they contain $\Gamma$ as well.}を以下のように定義する。
		\begin{equation*}
		\begin{array}{rcl}
		...
		\end{array}
		\end{equation*}
		このとき\footnote{Is there a particular reason we use subscript to indicate dependence on $\lambda,\mu,\nu$ in $a^{\ell,m}_{\lambda,\mu,\nu}$,
		but use brackets to indicate dependence on $\lambda,\mu,\nu$ in $b(\lambda,\mu,\nu)$? Maybe, it is better to write $a^{\ell,m}(\lambda,\mu,\nu)$?
		}\footnote{このとき seems to indicate the beginning of a sentence. But I do not see the dot ending the previous sentence. Is that OK?}、
...
\begin{equation*}
	...
	\end{equation*}
    を用いると、
    命題\ref{prop:exp-st-gg}は命題\ref{prop:exp-stz-gg}において$x=1$を代入したときの結果であるということが分かる.
...
\vspace{1em}
		...
		特別の場合は Warnaar, Varchenko, Tarasov などによる セルバーグ形の積分の一般化 とも関係する.
...
の $m=n=1,\alpha'=\beta',\alpha=\beta$ の特別な場合になる：\\
	...
	\begin{prop*}[$\ell=m=0$場合に帰着]\footnote{Note that this statement appeared as 補題 (Lemma) in slides, not as 命題 (Proposition).}
		\begin{equation*}
			\begin{array}[]{c}
...
\end{method}
\begin{remark*}
	Atle Selbergの元々のセルバーグ積分の証明\cite{Selberg:411367}はこのような手法が用いられ\doubt{た}。
\end{remark*}
...
%%ノンコンパクト部分群$G'$とその無限次元表現$\tau$を定める:\\
	...
	\begin{remark}
		...
		やや\doubt{緩}めて長さが有限の表現とする。また$G$および$G'$は
	...
	\begin{definition}
		連続な線形写像$A:\pi\to\tau$が$G'$絡作用素であるとき$A$を
...
\begin{goal*}\cite{kobayashi2015program}
	...
	完全に解決された。その時に用いられた手法を分\doubt{木してする}。
	\begin{enumerate}
		\item $\pi$と$\tau$は異なる
			空間に定義された
			表現であるが、例えば$(K,K')$-重複度が１の場合に（一般化した意味で）の\footnote
			{should we put の inside the bracket as well?}
			固有値を定義することができる；
		\item
...
という様子を\mykana{図示}{ずし}している。
図では異なる色は互\doubt{い}に同型でない
$K'$-タイプを表す;
\begin{figure}[h]
	\centering
...
\end{figure}
		...
		\item 従って、$(K,K')$固有値を積分の形で表示できる;
		\item 命題\ref{prop:int-st-gg}はガンマ関数の積公式として与えられるので
			ゼロかどうかが完全に決定できる.
	\end{enumerate}
