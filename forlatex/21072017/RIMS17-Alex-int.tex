...
% 文書情報
\title{２つのGegenbauer多項式に関連する積分公式について}
...
%%		最初のいくつは
	...
	$\lambda,\mu,\nu$の有理型関数$b(\lambda,\mu,\nu)$と、
		...
		\begin{array}{rcl}
		  ...
		  1 \right)}\text{\footnotemark}
		\end{array}
		\end{equation*}\footnotetext{maybe, dot here to indicate the end of a sentence?}
...
\begin{equation*}
	...
	を用いると、\begin{equation*}
		A^{\ell,m}_{\lambda,\mu,\nu}(1)=b(\lambda,\mu,\nu)a^{
		\ell,m}_{\lambda,\mu,\nu}
	\end{equation*}が成り立つ。従って\footnote{maybe, comma here?}
	定理\ref{prop:exp-st-gg}は命題\ref{prop:exp-stz-gg}において$x=1$を代入した特\doubt{殊}
	値\doubt{と}
    であるということが分かる。
...
\subsection{Selberg積分の特殊値との比較}\footnote{when I write an English word in Japanese
text, should I surround it by English spaces, like この Selberg 積分; surround it by 
Japanese spaces, like この　Selberg　積分; or put no spaces, like このSelberg積分?}
...
\end{equation*}とおく。
	 ...
	 \begin{equation*}
		...
		セルバーグ\footnote{we wrote セルバーグ in English (like, Selberg) before.
		Should we still keep the katakana here?}積分
の $k = 2,\alpha=\beta$ の場合
と同じ結果を与える。
\\
\myrelationdiagramBig{命題\ref{prop:int-st-gg}}{\ell=m=0,\\ \lambda=\mu}{k=2,\\
...
\cite[(1.4)]{warnaar2010sl3}\footnote{
  ...
  の $k_1=k_2 = 2,\alpha_1=\beta_1,\alpha_2=\beta_2$ の特別な場合と同じ結\doubt{果}を与える。\\
  \myrelationdiagramBig{命題\ref{prop:int-st-gg}}{\ell=m=0,\\
	...
	これは３つの自由パラメータ\footnote{should not it be ４つの自由パラメータ？}
...
の $k = 2,\beta_2=1,\alpha=\beta_1$ の特別な場合になる：\\
		...
		３つの関係式\eqref{eqn:DF3}を満たす
		６つのパラメータ$\alpha,\alpha',\beta,
		...
		\begin{equation*}
			\begin{array}{c}
				...
				\Psi_{k_1}(\alpha',\beta',\rho';\boldt)\Psi_{k_2}(\alpha,\beta
				,\rho;\bolds)
				\displaystyle\prod_{i,j=1}^{k_1,k_2}\myabs{t_i-\tau_j}^{-2}
				dtd\tau\mbox{\footnotemark} \\
				      \end{array}\end{equation*}
			
			      \begin{equation*}\footnotetext{maybe, it should be $\Psi_{k_1}(
				      \alpha'+1,\beta'+1,\rho';\boldt)$,$\Psi_{k_2}(
			      \alpha+1,\beta+1,\rho+1;\bolds)$?; also which variables should
...
\myrelationdiagramBig{命題\ref{prop:int-st-gg}}{\ell=m=0,\\
	...
	本セクションで、定理\ref{prop:exp-st-gg}の４通りの証明をあげる.
	...
	命題\ref{prop:exp-stz-gg}の証明においても同じアイディアが適用
	することができ
	\doubt{、}$\ell=m=0$の場合にその証明を帰着することができる。
	\end{proof*}
	\begin{method}[直接]
		\quad\\
	\begin{enumerate}
...
\begin{method}[命題\ref{prop:exp-stz-gg}によって]
	...
	まず、$\ell=m=0$場合に帰着する。すなわち\footnote{maybe, we need a comma here?}
			\begin{equation}
...
$K'$-タイプを表す。
対称性破れ作用素
は$G'$準同型なので\footnote{should we put a comma here?}特に$K'$準
同型であり\doubt{、}従って
、
上段と下段の
同じ色の間に写像が誘導される；
...
\end{figure}
	...
	の積分が\doubt{現}れた。命題\ref{prop:int-st-gg}では積分値
