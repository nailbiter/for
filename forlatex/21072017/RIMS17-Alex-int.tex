...
(世話人：阿部紀行), 京都大学数理解析研究所、2017年6月20日(火)--6月23日
(金)
の講演記録
}
...
\\
  ...
  + k_1 - k_2 - 1) \gamma) \Gamma (\gamma)} \times\mbox{\footnotemark}\\
  ...
  k_2 - 1) \gamma)}.
  \end{multline}
  \footnotetext{maybe, it is better to put newline \textit{before} the $\times$ sign?}
...
の $k = 2,\beta_2=1,\alpha=\beta_1$ の特別な場合になる：\\
  ...
  \rho)}{\Gamma (\rho)} \times\footnotemark\\
  \displaystyle\prod_{i, j = 0}^{n, m} \frac{1}{(\alpha + j \rho - i) (\beta + j \rho - i)
  (\alpha + \beta + \rho (m - 1 + j) - (n - 1 + i))} \times\footnotemark\\
...
\myrelationdiagramBigCenterSix{命題\ref{prop:int-st-gg}}{\ell=m=0,\\
		...
		\input{3star}
		\input{4star}
	更に、系\ref{cor:int-xzy-hh}の積分に$s=x-y$変数変換し、$w=1$をとれば、
