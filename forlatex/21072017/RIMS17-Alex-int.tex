...
\section{主結果}
	...
	\begin{example*}
		...
		\begin{eqnarray*}
			...
			C_4^\lambda(x)&=&\frac{1}{2}\lambda(1+\lambda)-2x^2\lambda(1+\lambda)(2
			+\lambda)+\frac{2}{3}\lambda(1+\lambda)(2+\lambda)(3+\lambda)x^4
		\end{eqnarray*}
	\end{example*}
...
エルミート多項式$\left\{ H_n \right\}_{n=1}^\infty$は、
	...
	\begin{fact*}[Mehta積分, {\cite{mehta2004random}}]
		...
		\;\footnote{I can describe the fact that Mehta integral and 系1 have common specialization
		with a sentence, like ``
系1の $w = 1, \ell = m = 0$ の特殊化がMehta積分
の $k = 2$ の特別な場合になります.
	'', but I thought diagram will be more expressive. Should I change back to verbal explanation (here and below)?}
