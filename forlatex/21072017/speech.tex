...
Gegenbauer多項式は様々な同値定義があり、例えば
Gegenbauer多項式は、以下に述べるGegenbauerの微分方程式を満たす多項式です。
%%コンパクト群$O(n)$の分規則ルールの具体的な形を調べるとき、Gegenbauer多項式も出て来ます。
球面調和関数の\kana{記述}{きじゅつ}にGegenbauer多項式が現れる
というのが、もともとの\kana{由来}{ゆらい}です。
...
シューアの補題を用いることで、
対称性破れ作用素の\kana{対角化}{タイカクカ}を行うことができます。
この図は、上の段では、それぞれの$K$-typeがK'の表現として
multiplicity-freeに分解されている様子を表し、
対称性破れ作用素によって、同じ色の間に写像が誘導される
という様子を\kana{図示}{ずし}しています。
