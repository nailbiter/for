...
%%
\slide
最初は、主結果のご紹介のため、Gegenbauer多項式という多項式のファミリの定義と性質を復習したいと思います。
...
一般な結果で
\kana{係数}{ケイスウ}は単純な$\Gamma$関数の積でなく、超幾何関数$_{2}F_1$も出て来るので、ご注意ください。

\slide
命題１は実際には色々な同型形があります。
例えば、命題$1'$に記述されたように、積分公式として述べられます。
\kana{換言}{カンゲン}すれば、命題$1'$の積分公式は命題$1$の展開公式と同値です。\\
もちろん、最も一般の命題２の展開公式も積分公式として述べられますが、ここで発表の記述を簡単にするため、
ご省略させて頂きます。
...
エルミート多項式というのは、
$\mathbb{R}$空間上で\kana{重み}{オモミ}関数 $e^{-x^2/2}$の直交多項式のファミリです。\\
しかも、エルミート多項式を\kana{上記}{ジョウキ}でご紹介されたGegenbauer多項式の極限とみなせます。
...
最初は、ここにいらっしゃる皆様がよくご存知
セルバーグの1944年の結果からご紹介始めたいと思います。
...
\slide%HR
最後の関連結果として、系１の公式に積分変換すれば、２つHermite-Rodriguez関数の\kana{畳み込み}{タタミコミ}の
メリン変換の明示公式を得ます。
...
\slide%M1
それでは、命題１の最初の証明方法をご紹介します。
前のスライドに述べたように、$\ell=m=0$の特別な場合を示せば良いです。
{超幾何関数のオイラー積分表示}を用い、
次に級数展開を用いて、求める積分を${}_3F_2$ \kana{高階}{コウカイ}
超幾何関数で表せます。こうして、Whipple sumによって、得られた${}_3F_2$超幾何関数を$\Gamma$関数の積で表せます。

\slide%M2
よく知られているCarlson定理という強い結果を認めれば、最も簡単な証明を\kana{提供}{テイキョウ}できます。
もし、あまりご存知でない方がいらっしゃれば、Carlson定理というのは、exponential typeという正則関数の広いクラスに関する定理です。
皆さんはよくご存知通りに、普通の\kana{右半面}{ミギハンメン}上での正則関数は、整実数上でゼロであれば、\kana{恒等的}{コウトウテキ}に0なります。
しかし、exponential typeであれば、更にもう一個条件を満たせば、同じ結論は整数上だけの情報から従うということがCarlsonの定理に明らかにされています。

%%\newcommand{\slide}{\noindent\stepcounter{framecount}$\backslash\backslash$[\arabic{framecount}]\\}
%\newcounter{framecount}
\slide
この結果を認めれば、命題$1'$の積分等式の$\nu$は整数であるの特別な場合を示せば良いということが分かります。
この場合で、$s+t$のベキ乗が多項式になって、左辺の積分がBeta積分の有限和になって、簡単に計算できます。\\
面白いですが、アトル・セルバークの元々のセルバーグ積分の証明はこのようなメソッドを使っていました。

\slide
次は、３つ目のアプローチをご紹介したいと思います。
流れ的には前に説明されたメスッド１と似ていますが、示すのは
命題１じゃなくて、\kana{最}{モット}も一般的な命題２です。まず、メソッド１のように、
$\ell=m=0$の場合（つまり、一番上の積分公式）を示せば良いです。\\
オイラー積分表示を用いり、${}_2F_1$超幾何関数の二次変換を用いると
...
性質を詳しく調べようという問題を考えましょう。

\slide
ある無限次元表現のクラスに対して（例えば、
主系列表現）、その作用素は超関数核を持つ積分変換で表せます。
...
今の図式で表されているような条件になります。
ここで、別様色が別様な$K'$-タイプを表します。

\slide
intertwining作用素の固有値がいつゼロになるかどうかを決定することが鍵になります。
表現空間
\kana{間}{アイダ}の作用素は積分核で表されているので、
固有値は積分で表せます。
...
%%KからK'への制限も multiplicity-freeという仮定のもとでは、
%%シューアの補題を用いることで、
%%対称性破れ作用素の対角化を行うことができます。
%%この図は、上の段では、それぞれのKtypeがK'の表現として
...
%%Page 1
%%
%%Gegenbauer多項式はGegenbauerの微分方程式を満たしますが、
%%Gegenbauer の微分方程式をみたす多項式はGengenbauerの多項式とは限りません。
%%これは Kobayashi-Pevznerの論文を講演終了後、良く読んでください
%%
%%「従って、Gegenbauer多項式は様々な同値定義があり、例えば
%%以下の微分方程式の特別な解として定義することもできます。」
%%は間違いです。
%%
%%「Gegenbauer多項式は、以下に述べるGegenbauerの微分方程式を満たす多項式で
%%す。」
%%
%%と話せば正しい文章になります。
%%
...
%%「
%%コンパクト群O(n)
%%の分規則ルールの具体的な形を調べるとき、Gegenbauer 多項式も出て来ます。」
%%
%%
%%また「分岐則」という言葉は、冒頭で決して話してはいけないという指導を以前
%%に行いました。
%%しかも、数学的に間違ったことを書いています。
%%
%%O(n)の分岐則でGegenbauer多項式が現れるのは、球面調和関数における表現とい
%%う特別な場合だけです。
%%これは内容が間違いです。
%%
%%
%%「球面調和関数の記述（きじゅつ）にGegenbauer多項式が現れる
%%というのが、もともとの由来（ゆらい）です。」
%%
%%とのみ話せばよいでしょう。
%%
%%
...
%%Page 6
%%
%%Mano Gen 先生→ Mano Gen さん
%%
%%
%%PAge 29
%%スライドに正しい日本語を書いたのですから、該当箇所はその文章を朗読するほ
%%うが良いでしょう。
%%
%%小林俊行
