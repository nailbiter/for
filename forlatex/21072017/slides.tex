...
\end{tikzpicture}
\end{frame}
\begin{frame}[fragile]{Hierarchy (Selberg'44)}
%%	\begin{description}
%%		\item[Warnaar'10] 
%%		\item[Tarasov-Varchenko'03]
%%		\item[Dotsenko-Fateev'85]
	\fbox{Selberg'44} is the famous Selberg integral \cite{Selberg:411367}\begin{equation*}
		\begin{array}[]{c}
				\int_{t \in [0, 1]^k} \Pi^{\alpha - 1, \beta - 1} (t) \Delta^{2 \gamma} (t) d
				t,\\
				\Pi^{\alpha-1,\beta-1}(t):=\displaystyle\prod\displaylimits_{i=1}^k t_i^{\alpha-1}(1-t_i)^{\beta-1},\\
				\Delta^{2\gamma}(t):=\displaystyle\prod\displaylimits_{1\le i<j\le k}\myabs{t_i-t_j}^{2\gamma},\\
				\tmop{Re} (\alpha), \tmop{Re} (\beta) > 0, | \gamma | \ll 1
		\end{array}
			\end{equation*}
			and it's particular case $k=2,\alpha=\beta$ with LHS being \begin{equation*}
				\iint_{[0, 1]^2} s^{\alpha-1}t^{\alpha-1}(1-t)^{\alpha-1}(1-s)^{\alpha-1} \myabs{s-t}^{2\gamma} dsdt
			\end{equation*}
...
\begin{frame}{Hierarchy (Warnaar'10)}
	...
	\fbox{Warnaar'10} is the generalization of Selberg integral shown in \cite[(1.4)]{warnaar2010sl3}\begin{equation*}
			\begin{array}{c}
				  \int_{(s, t) \in C_{\beta_1, \gamma}^{k_1, k_2} [0, 1]} \Pi^{\alpha_1 - 1,
						...
						+ (i - k_2) \gamma) \Gamma (( i + 1) \gamma)}{\Gamma (\alpha_1 + \beta_1 + (i
							  + k_1 - k_2 - 1) \gamma) \Gamma (\gamma)} \\
						\times\displaystyle\prod_{i = 0}^{k_2 - 1} \frac{\Gamma (\alpha_2 + i \gamma) \Gamma (\beta_2 +
			...
			\end{array}
			\end{equation*}
			and $C^{k_1,k_2}_{\beta_1,\gamma}[0,1]$ is some {\it linear sum} of subsets of $[0,1]^{k_1+k_2}$.
			The particular case $k_1=k_2=1,\alpha_1=\beta_1,\alpha_2=\beta_2$ with LHS being\begin{equation*}
				\left(\displaystyle\iint_{\begin{array}[]{c}
					[0,1]^2\\t<s
				\end{array}}+\frac{\sin(\pi\alpha_1)}{\sin(\pi\alpha_2)}\displaystyle\iint_{\begin{array}[]{c}
					[0,1]^2\\t>s
