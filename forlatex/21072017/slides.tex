...
\newtheorem{taggedredpropx}{\underline{fuck}}
\newtheorem{remark}{注}
\newtheorem{cor}{系}
%%\newtheorem*{proof}{証明}
\newtheorem*{example*}{例}
...
\newenvironment{taggedprop}[1]
 {\renewcommand\thetaggedpropx{#1}\taggedpropx}
  {\endtaggedpropx}
\newenvironment{taggedredprop}[1]
 {\renewcommand\thetaggedredpropx{#1}\taggedredpropx}
  {\endtaggedredpropx}
...
\begin{frame}[fragile]
	...
	\begin{textblock*}{\textwidth}(3.99cm,1.7cm)
	  	\begin{tikzpicture}
			...
			\node at (-0.25,0.0) {\color{blue}{\tiny$\begin{array}[]{c}
				k=2,\\
			\alpha=\beta
		\end{array}$}};
		\end{tikzpicture}
	...
	\begin{textblock*}{\textwidth}(1cm,0.8cm)
	  	\begin{tikzpicture}
			...
			\node at (2.8,3.4) {\color{red}{\tiny$\begin{array}[]{c}\ell=m=0,\\ \lambda=\mu \end{array}$}};
		\end{tikzpicture}
	\end{textblock*}
...
%%					    \\ \times
		...
		\begin{equation*}
			\begin{array}{c}
					  ...
					  \Delta^{2\gamma}(t):=\displaystyle\prod_{1\le i<j\le k_1 }\myabs{t_i}^{2\gamma}, \Delta^\gamma(t,s):=\displaystyle\prod\displaylimits_{i,j=}^{k_1,k_2}
					  \myabs{t_i-s_j}^{\gamma},\\
					  \\
	...
	\begin{textblock*}{\textwidth}(1cm,0.7cm)
	  	\begin{tikzpicture}
			\draw[->,>=angle 90,color=red,style=very thick] (2.0,4.8) -- node{} (2.0,2.0);
			\node at (2.8,3.4) {\color{red}{\tiny$\begin{array}[]{c}\ell=m=0,\\ \lambda=\mu \end{array}$}};
		\end{tikzpicture}
	\end{textblock*}
	\begin{textblock*}{0.5\textwidth}(-0.4cm,3.7cm)
