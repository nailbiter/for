...
\begin{frame}{Method 1 (direct)}
	...
	\begin{enumerate}
		\item Using the property $
				...
				$, where $a(\alpha,n):=$\\$
				\frac{(-1)^n}{2^nn!}\frac{\Gamma\left( \alpha+\frac{1}{2} \right)\Gamma\left( n+2\alpha \right)}{\Gamma(2\alpha)\Gamma\left(\alpha+n+\frac{1}{2}  \right)}
				$ of Gegenbauer polynomials and integration by parts we have $\mbox{LHS}\eqref{eqn:int-st-gg}=$\begin{equation*}
					\kern-1cm\mbox{LHS}\eqref{eqn:int-st-gg}=a(\lambda,l)a(\mu,m)
					...
					d s d t=a(\lambda,l)a(\mu,m)\iint_{[-1,1]^2}\myabs{s-t}^{2\nu-l-m}(1-s^2)^{\lambda+n-\frac{1}{2}}(1-t^2)^{\mu+m-\frac{1}{2}}dsdt;
				\end{equation*}
	\end{enumerate}
\end{frame}
\begin{frame}{Method 1 (direct)}
