...
\begin{frame}
	Furthermore,
	Letting $\lambda,\mu\to\infty$ (while keeping $\mu/\lambda=z\in\mathbb{R}$) in Proposition \ref{prop:exp-st-gg}, we arrive at:
	\begin{cor}\label{cor:int-xzy-hh}
		For $\nu\in\mathbb{C}$, $z\in\R$ the following holds:
		\begin{equation*}
			\begin{array}[]{c}
	...
	\end{cor}
	\begin{remark}
		The particular case $k=2$ of \underline{Mehta integral} follows from this (take $z=1,\ell=m=0$).
	\end{remark}
	Even more,
\end{frame}
\note{TODO (slides):\begin{enumerate}
\item elaborate what limit was taken;
\item emphasize that $z$ is no longer in $[-1,1]$ anymore;
\end{enumerate}
TODO (speech):\begin{enumerate}
	\item keep in mind the formula \begin{equation*}
...
\end{frame}
\note{TODO (slides):\begin{enumerate}
\item is the word ``harder'' appropriate?
