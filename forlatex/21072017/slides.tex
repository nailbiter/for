	...
	\begin{enumerate}
		\item Using the property $
			...
			\item Upon change of variables $s=(1-t)(1-w)+t$ and using Euler's integral representation 
					\vspace{-0.2cm}
				\begin{equation*}
					\textstyle B(b,c-b){}_2F_1(\begin{array}[]{c}
						a,b\\c
					\end{array};z)=\textstyle\int_{0\le x\le1}x^{b-1}(1-x)^{c-b-1}(1-zx)^{-a}dx,\quad\Re(c)>\Re(b)>0
					\vspace{-0.2cm}
				\end{equation*}
				of ${}_2F_1$
...
\begin{frame}{Method 1 (direct)}
	...
	\begin{enumerate}
		\item In fact, \eqref{eqn:int-st-gg} follows from the special case $\ell=m=0$:
			...
			\end{equation*}
		\item Upon change of variables $w=s-t$, the inner integral (in $w$) can be evaluated via Euler's integral representation of ${}_2F_1$;
%%			via \cite[ET II 186(9)]{gradshteinryzhik}:
%%		\begin{equation*}
%%			\kern-1cm\int_0^u x^{\nu - 1} (x + \alpha)^{\lambda} (u - x)^{\mu - 1} d x =
%%			\alpha^{\lambda} u^{\mu + \nu - 1} B (\mu, \nu)_2 F_1 \left( - \lambda, \nu ;
