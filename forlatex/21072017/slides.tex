...
%%		\end{array}\]
	Classify all $G'$-intertwining operators $A:\pi\to\tau$.
	\end{block}
	{\bf Idea:}\begin{itemize}
		\item Although $\pi$ and $\tau$ are realized on different spaces, we can define ``generalized eigenvalues'';
		\item it turns out that any $A$ is ``diagonal'';
		\item $\implies$ it would help to compute eigenvalues explicitly;
		\item $A:\pi\to\tau$ are given by integral kernels $\implies$ eigenvalues are given by integral equalities;
	\end{itemize}
\end{frame}
}
\begin{frame}
...
%%	\footnote[frame]{I remember, You said the diagram is 
