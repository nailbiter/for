...
\begin{frame}[fragile]{Hierarchy (1/2)}
\begin{tikzpicture}
\input{intdep.tex}
\end{tikzpicture}
\end{frame}
\begin{frame}[fragile]{Hierarchy (2/2)}
	\begin{description}
		\item[Warnaar'10] 
		\item[Tarasov-Varchenko'03]
		\item[Dotsenko-Fateev'85]
		\item[Selberg'44] is the famous Selberg integral\begin{equation*}
				\int_{t \in [0, 1]^k} \Pi^{\alpha - 1, \beta - 1} (t) \Delta^{2 \gamma} (t) d
				t,\quad \Delta(t):=\prod_{i<j}\myabs{t_i-t_j}
			\end{equation*}
