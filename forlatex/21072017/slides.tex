...
\begin{frame}
	Furthermore,
	Letting $\lambda,\mu\to\infty$ (while keeping $\mu/\lambda=w\in\mathbb{R}$) and using the limit formula
	\vspace{-0.4cm}
	\begin{equation*}
	...
	\begin{remark}
		...
		\begin{equation*}
			\frac{1}{(2\pi)^{k/2}}\int_{\mathbb{R}^k}\myabs{\Delta(\mathbf{t})}^{2\gamma}\exp\left( -\myabs{\mathbf{t}}^2/2 \right)\mathbf{t}d
		\end{equation*}
