...
\begin{frame}{Hierarchy (Warnaar'10)}
	\fbox{Warnaar'10} is the generalization of Selberg integral shown in \cite{warnaar2010sl3}\begin{equation*}
			\begin{array}{l}
				  \int_{(s, t) \in C_{\beta_1, \gamma}^{k_1, k_2} [0, 1]} \Pi^{\alpha_1 - 1,
					    \beta_1 - 1} (t) \Pi^{\alpha_2 - 1, \beta_2 - 1} (s) \Delta^{2 \gamma} (t)
					      \Delta^{2 \gamma} (s) \Delta^{- \gamma} (t, s) d s d t\\
					        = \prod_{i = 0}^{k_1 - 1} \frac{\Gamma (\alpha_1 + i \gamma) \Gamma (\beta_1
						+ (i - k_2) \gamma) \Gamma (( i + 1) \gamma)}{\Gamma (\alpha_1 + \beta_1 + (i
							  + k_1 - k_2 - 1) \gamma) \Gamma (\gamma)} \times\\
%%							    \prod_{i = 0}^{k_2 - 1} \frac{\Gamma (\alpha_2 + i \gamma) \Gamma (\beta_2 +
%%							    i \gamma) \Gamma \left( i + 1) \gamma)}{\Gamma (\alpha_2 + \beta_2 + (i + k_2 -
%%								      k_1 - 1) \gamma) \Gamma (\gamma)} \prod_{i = 0}^{k_1 - 1} \frac{\Gamma
...
%%												  \gamma \nin \mathbbm{Z}
			\end{array}
			\end{equation*}
			and it's particular case $k=2,\alpha=\beta$\begin{equation*}
				\iint_{[0, 1]^2} s^{\alpha-1}t^{\alpha-1}(1-t)^{\alpha-1}(1-s)^{\alpha-1} \myabs{s-t}^{2\gamma} dsdt
			\end{equation*}
			is generalized by Proposition \ref{prop:int-st-gg}.
\end{frame}
