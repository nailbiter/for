...
\begin{frame}{Method 1 (direct)}
	...
	\begin{enumerate}
		\item Using the property $
				...
				\end{equation*}
		\item Upon change of variables $s=(1-t)(1-w)+t$, the inner integral (in $w$) can be evaluated via Euler's integral representation of hypergeometric function as
			\begin{equation*}
				\iint\displaylimits_{[-1,1]^2}\kern-0.2cm\myabs{s-t}^{2\nu-l-m}(1-s^2)^{\lambda+l-\frac{1}{2}}(1-t^2)^{\mu+m-\frac{1}{2}}dsdt=
				\int_{- 1}^1 (s - z)_+^{2 c - 1} (1 - s^2)^{a - 1} d s =\int_{-1}^1 
				2^{\lambda+l-\frac{1}{2}} B \left( 2\nu-l-m+1, \lambda+l-\frac{1}{2} \right)
				(1 - z)^{2 \nu+\lambda-m+\frac{3}{2}} {}_2 F_1 \left( \begin{array}{c}
					\frac{1}{2}-l-\lambda, 2\nu-l-m+1\\
					2 \nu+\lambda-m+\frac{3}{2}
