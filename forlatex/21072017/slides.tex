...
\begin{frame}
	\begin{prop}\label{prop:exp-st-gg}
		...
		{
		\begin{equation*}
	a_{\lambda,\mu,\nu}^{\ell,m}\kern-0.1cm:= { (\lambda + \ell) (\mu + m)   }{\Gamma^{-1} \kern-0.1cm\left( \lambda + \nu + \frac{\ell -
  m}{2} + 1 \right)  \kern-0.1cm\Gamma^{-1} \kern-0.1cm\left( \mu + \nu -
  \frac{\ell - m}{2} + 1 \right)\kern-0.1cm\Gamma^{-1} \kern-0.1cm\left( \lambda + \mu + \nu + \frac{\ell +
  m}{2} + 1 \right)\Gamma^{-1}\left(  \nu+1-\frac{\ell+m}{2}\right)},\end{equation*}
  \begin{equation*}
  b(\lambda,\mu,\nu):=2^{-2\nu}\Gamma (\lambda + \mu + 2 \nu + 1){\Gamma (\lambda)
...
1 \right)}
	...
	\end{prop}
	{\bf Plan:} give 4 independent proofs of this result.
...
%%		(1-x^2)y''-(2\lambda+1)xy'+n(n+\lambda)y=0.
%%	\end{equation*}
	Hermite polynomials are the family of polynomials $\left\{ H_n \right\}_{n=1}^\infty$ that form an orthogonal basis of $L^2\left( \mathbb{R},e^{-\frac{x^2}{2}}dx \right)$.
	\begin{example*}
		The first few of them are:
		\begin{eqnarray*}
		H_0(x)&=& 1;\\
