...
\end{enumerate}}
\begin{frame}{Method 4 (easy, direct)\footnote{I am starting to think that this method is the same as Method 1}}
	We prove the Proposition \ref{prop:int-st-gg} (with $t$ replaced by $-t$) directly:
	\begin{enumerate}
		\item We first take the inner integral, using the formula \cite[7.4.11]{kobayashi2011schrodinger}
			\begin{equation*}
				\kern-1.5cm\int_{-x}^1(x+y)^\lambda \tilde{C}_k^\nu(y)(1-y^2)^{\nu-\frac{1}{2}}dy=\frac{\sqrt{\pi}\Gamma(2\nu+k)\Gamma(\lambda+1)}{2^{\nu-\frac{1}{2}}k!}
...
\end{frame}
\note{TODO (slides):\begin{enumerate}
	\item change normalization of Gegenbauer polynomial in first step's formula; change parameters and variables' names
\end{enumerate}
}
