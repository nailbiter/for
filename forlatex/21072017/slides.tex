...
\begin{frame}{Method 4 (direct, using non-trivial integral formul\ae)}
	...
	\begin{enumerate}
		...
		\item The remaining integral then is then of the form\begin{equation*}
				\int_{-1}^1(1-s^2)^{\nu+\frac{\mu}{2}+\lambda-\frac{1}{4}}P_{\mu+m-\frac{1}{2}}^{-2\nu-\mu-\frac{1}{2}}(-s)C^{\lambda-\frac{1}{2}}_\ell(s)ds
			\end{equation*} 
		\item Use integration by parts together with the formul\ae\;$\left(C^\lambda_\ell(x)  \right)'=C^{\lambda+1}_{\ell-1}(x),\;\left((1-x^2)^{-a/2}P^a_b(x) \right)=-(1-x^2)^{-\frac{a+1}{2}}P_b^{a+1}$
		\item and is readily handled by \cite[7.132.1]{gradshteinryzhik}.
		\item 
	\end{enumerate}
\end{frame}
\note{TODO (slides):\begin{enumerate}
