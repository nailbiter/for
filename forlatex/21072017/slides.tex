...
\end{frame}
\section{主結果}
\begin{frame}{Gegenbauer 多項式}%gegenbauer
...
\section{証明}
\begin{frame}
	以下の述べられているメソッド１とメソッド３は最初のステップとして以下のような補題を使う\footnote{is 削減 a correct word for ``reduction''?}:
...
\begin{frame}{メソッド 1 (直接) (1/2)}
	...
	\begin{enumerate}
		...
		\item $s=(1-t)(1-w)+t$積分変換と${}_2F_1$超幾何関数のオイラー積分表現\footnote{is オイラー積分表現 a correct term?}
					\vspace{-0.2cm}
				\begin{equation*}
...
\end{enumerate}}
\begin{frame}{メソッド 2 (\only<1>{1}\only<2>{2}/2)}
			\vspace{-0.2cm}
	...
	\begin{remark}
		\begin{equation*}
			...
			f(z)=\sin(\pi z)
		\end{equation*}
		という関数が科目２を除いで、Carlsonの定理の全ての条件を満たす.特に、
		$f\big|_{\Z}=0$が成り立つ.しかし、 $f(z)$がゼロ定数関数\footnote{is 定数関数 correct for ``constant function''}ではない。

		よって、Carlsonの定理の科目２の条件が必要である.
	\end{remark}
	}
	\only<2>{
...
\item 
\end{enumerate}}
\begin{frame}{メソッド 3 (proving Prop. \ref{prop:exp-stz-gg})}
	\scriptsize
...
\end{enumerate}}
\begin{frame}{メソッド 4 (using non-trivial integral formul\ae\;and Legendre functions)}
	\scriptsize
