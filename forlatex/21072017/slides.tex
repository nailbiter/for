...
\begin{frame}{Gegenbauer 多項式}%gegenbauer
	...
	、
	$\Re(\lambda)>-\frac{1}{2}$のとき$L^2\left( [-1,1],(1-x^2)^{\lambda-\frac{1}{2}}dx \right)$空間の直交多項式\footnote{is 直交多項式 a correct japanese word?}である.
	\vspace{-0.2cm}
	\begin{example*}
		最初のいくつは
	\vspace{-0.3cm}
...
\begin{frame}
	...
	制限をとれば($\mu/\lambda=w\in\mathbb{R}$を保つ\footnote{or maybe 守る？}ように)、以下の主張を得る:
...
\begin{frame}{メソッド 1 (直接) (1/2)}
	...
	\begin{enumerate}
		...
		\item $s=(1-t)(1-w)+t$積分変換とオイラー積分表現\footnote{is オイラー積分表現 a correct term?}
					\vspace{-0.2cm}
				\begin{equation*}
