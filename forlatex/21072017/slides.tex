...
\item find some nontrivial case of Proposition 1 which {\it does not} require $\ell=m=0$;
\end{enumerate}
}
\begin{frame}
	\begin{figure}[h]
...
\item Taking the outer integral, we arrive at hypergeometric $_3F_2(;1)$ function which simplifies via Whipple's sum.
	\end{enumerate}
\end{frame}
\note{TODO (slides):\begin{enumerate}
...
\end{frame}
\note{FIXME (slides):\begin{enumerate}
	...
	\begin{enumerate}
		\item %\label{enum:m4-1}
			...
			using the equality {\scriptsize \begin{equation}
				\kern-1.5cm 2\int_{- 1}^1\kern-0.5cm \int_{- 1}^1 \left( s-tz \right)_+^{2c-1}  (1 - s^2)^{a - 1} (1 -
			...
			\begin{array}{c}
				...
				\label{eqn:prop21}
			\end{equation}}
		\item The inner integral (with respect to $s$) is done via variable change $s=(1-tz)(1-w)+tz$ and Euler's integral representation of ${}_2F_1$:\begin{equation*}
				\int_{- 1}^1 (s - z)_+^{2 c - 1} (1 - s^2)^{a - 1} d s = 2^{a - 1} B (2 c, a)
...
\end{frame}
\note{TODO (slides):\begin{enumerate}
\item are the words ``harder/easy'' appropriate?
