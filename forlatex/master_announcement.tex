...
\begin{equation*}
	\Hom_{G'}\left( \left( \pi' \right)^\infty,\pi^\infty_{G'} \right)
\end{equation*}
...
of the $G$-equivariant line bundle $G\times_{P}\left( \chi_\lambda,\C \right)\to G/P$, so that $I(\lambda)$ itself is the smooth Fr\'echet globalization of moderate growth.
\underline{
The parametrization is chosen in a way that $I(\lambda)$ contains a finite-dimensional representation as a subrepresentation if $-\lambda\in\N$ and as a quotient if $\lambda-\left( p+q\right)\in\N$.}
%%\begin{eqnarray}
%%  & I (\lambda) , &  \nonumber\\
