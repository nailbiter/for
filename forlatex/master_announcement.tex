...
of $C^\infty$-vectors of $\mathcal{H}_\pi$ is naturally endowed with Fr\'echet topology, and $(\pi,\mathcal{H}_\pi)$ gives rise to a 
\uwave{continuous (was ``conditions'' originally)} representation $\pi^\infty$
...
\end{theorem}
We shall construct explicitly all the symmetry breaking operators in the Section \ref{sec:constr}.
...
\begin{equation*}
	X:=\Xi^{p+1,q+1}/\R^{\times}\simeq \left( \Sp^p\times\Sp^q \right)/\Z_2.
\end{equation*}
...
\begin{theorem}[description of $P'\backslash G/P$]%[classification of closed $P'$-invariant subsets of $G/P$]
	...
	\end{array}
	$
	means that $A\supset B$ and that the subvariety of $B$ is of codimension $m$ in $A$.\\
  \begin{figure}[h]
    \hspace{-0.6cm}
...
The slice of $\Xi^{p+1,q+1}$ by the hyperplane $x_0-y_q=2$ \uwave{(I think it should be $x_0+y_q=0$, compare with \cite[(4.9)]{kobayashi2015symmetry} (attached))}
defines the coordinates $(x,y)\in\R^{p+q}$ of the open Bruhat cell $U$ of $G/P$, and induces the $N$-picture of the representation $I(\lambda)$\begin{equation*}
	\iota_\lambda^*:I(\lambda)\hookrightarrow C^\infty(\R^{p+q})
\end{equation*}via the trivialization of the line bundle $\mathcal{L}_\lambda\simeq\R^{p+q}\times\C$.

We shall realize
a symmetry breaking operator $T$ in the $N$-pictures of $I(\lambda)$ and $J(\nu)$, and
find a distribution $K_T\in\mathcal{D}'\left( \R^{p+q} \right)$ such that $\iota^*_\nu(Tf)(x')$ is given by
\begin{equation*}
...
\end{figure}
for all $f\in I(\lambda)$, where $x',y'\in\R^{p+q-1}$. 
...
conditions for the distribution kernels
%%space $\SBO$ 
...
\begin{definition}\label{def2}We let $O(p-1,q)$ act on $\R^n$ $(n=p+q)$ by leaving $x_p$ invariant. We define $\sol$ 
	to be the space of distributions $F\in\mathcal{D}'(\R^n)$ satisfying the following four conditions:
\begin{enumerate}[(1)]
    \item $F (x) = F (- x)$;
    \item $F$ is $O(p-1,q)$-invariant;
    \item $F$ is homogeneous of degree $\lambda-\nu-n$;
    \item $F$ is $N_+'$-invariant (see Definition \ref{def1}).%on $\R^{p,q}$.
  \end{enumerate}
\end{definition}
%%\end{versiona}
...
By \cite[Sect. 2]{kobayashi2016differential1}, $T$ is a {\it differential} symmetry breaking operator if and only if $\Supp(T)$ is a singleton. (\uwave{We need to
define $\Supp$}.)
...
For each closed subset $S$ of $P'\backslash G/P$, we construct a family $R^S_{\lambda,\nu}$ of SBOs, such that:
\begin{itemize}
...
\item 
	...
	\end{enumerate}
\end{theorem}
We shall renormalize $R_{\lambda,\nu}^X$ for $(\lambda,\nu)$ such that $R_{\lambda,\nu}^X$ vanishes. For this, we begin with
an observation that $\Gamma\left( \frac{\lambda-\nu}{2} \right)$ is holomorphic in $\C^2\oldsetminus //$, \uwave{(I think ``-'' is better that $\oldsetminus$ here, 
as we use ``-'' for set difference below)} and therefore\begin{equation*}
	\begin{array}[]{ll}
	...
	\end{array}
\end{equation*}
for $(\lambda,\nu)\in\C^2\oldsetminus //$. Moreover, in light \uwave{of the fact} that $K_{\lambda,\nu}^X$ vanishes on $\mathcal{A}=\mid\mid\mid\cap//$ by
Theorem \ref{thm:regular} (2), we obtain its analytic continuation on $\mid\mid\mid$ as follows.\begin{theorem}[renormalized operator $\tilde{R}_{\lambda,\nu}^X$]
	\begin{enumerate}[(1)]
		\item The renormalized symmetry breaking operator\begin{equation*}
			...
			\end{equation*}is defined for $(\lambda,\nu)\in\mid\mid\mid$ that depends holomorphically in \uwave{(I think ``on'' is better)} $\lambda\in\C$.
		\item $\tilde{R}^X_{\lambda,\nu}$ vanishes if and only if $p=1$, $q$ even and $(\lambda,\nu)\in\mathcal{X}\oldsetminus//$.
			(\uwave{note that $\mathcal{X}$ is undefined at this point; it is defined below})
	\end{enumerate}
	\label{thm:X2}
\end{theorem}
...
%%	By definition, $R_{\lambda,\nu}^{ \left\{ o \right\}}$ in Theorem \ref{thm:construction} amounts to
	...
	The coefficients $a_j(\lambda,\nu)$ gives rise to a polynomial\begin{equation*}
		\tilde{C}_{2l}^{\lambda+\frac{n-1}{2}}(s,t)=\sum_{j=0}^la_j(\lambda,\nu)s^jt^{2l-2j}
	\end{equation*}such that $\tilde{C}_{2l}^{\lambda+\frac{n-1}{2}}(s,t)$ 
	(\uwave{I think I did not completely understand Your writing was it ``$\tilde{C}_{2l}^{\lambda+\frac{n-1}{2}}(s,t)$'' or
	``$\tilde{C}_{2l}^{\lambda+\frac{n-1}{2}}(1,t)$''?})
	is a renormalized Gegenbauer polynomial \cite[(16.3)]{kobayashi2015symmetry}, and $R_{\lambda,\nu}^{ \left\{ o \right\}}=\Op\left( K_{\lambda,\nu}^{ \left\{ o \right\}} \right)$ with
	\begin{equation*}
		K_{\lambda,\nu}^{ \left\{ o \right\} }:=\tilde{C}_{2l}^{\lambda+\frac{n-1}{2}}\left( -\Delta_{\R^{p-1,q}}\delta_{\R^{p+q-1}},\delta(x_p) \right).
	\end{equation*}
	The operator $R_{\lambda,\nu}^{\left\{ o \right\}}$
	was previously found in \cite[Thms. 5.1.1 and 5.2.1]{juhl2009families}, \cite[(10.1)]{kobayashi2015symmetry} for $q=0$ and in \cite[Thm.\ 4.3]{kobayashi2015branching}
	for general $p,q$.
\end{fact}
