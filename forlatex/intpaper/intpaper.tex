...
\begin{proof*}{Proof}
	...
	\end{equation*}we see Lemma \ref{lem:1718109} from \eqref{eqn:vnlmd} and from the asymptotic behaviour of the $\Gamma$ function:\footnote{
		I am not sure I understand the proof. When $\lambda>0$ we have $\mynorm{C_n^\lambda}_{L^\infty}=
		\frac{\Gamma(n+2\lambda)}{n!\Gamma(2\lambda)}\sim
		n^{2\lambda-1}$ and $\mynorm{C^\lambda_n}_{L_\lambda^2}=\sqrt{\frac{\pi2^{1-2\lambda}\Gamma(n+2\lambda)}{n!(n+\lambda)\Gamma(2\lambda)}}
		\sim\sqrt{\frac{n^{2\lambda-1}}{n}}\sim n^{\lambda-1}$ and we have
		$\frac{\mynorm{C_n^\lambda}_{L^\infty}}{\mynorm{C^\lambda_n}_{L_\lambda^2}}\sim n^\lambda\to\infty$, so that
		$\frac{\mynorm{C_n^\lambda}_{L^\infty}}{\mynorm{C^\lambda_n}_{L_\lambda^2}}$ is unbounded for big $n$. How do we bound it by $d_{\lambda,n}$
		then?
	}\begin{equation*}
