...
\section{Sobolev-type estimates for Gegenbauer expansion}\label{sec:Sobolev}
...
\begin{lemma}
	Suppose $\lambda\neq0$ and $\lambda>-\frac{1}{2}$. Let $N\in\N$. Then there is $d_{\lambda,N}$ such that \begin{equation*}
...
\begin{proof*}{Proof of Proposition \ref{prop:1718105}.}
	...
	\end{equation*}Thus, for all $n\ge N$, we have\begin{equation*}
		\myabs{a_n(f)}\le\frac{1}{2^N(l)_N}\frac{\mynorm{f^{(N)}(x)}_{L^2_{\lambda+N}}}{C^{\lambda+N}_{n-N}(x)}_{L^2_{\lambda+N}}.
	\end{equation*}By Lemme \ref{lem:1718109}\begin{equation*}
		\myabs{a_n(f)}\mynorm{C^\lambda_n(x)}_{L^\infty(-1,1)}\le\frac{d_\lambda}{2^N(\lambda)_N}\mynorm{f^{(N)}(x)}_{L^2_{\lambda+N}}n^{\lambda-N}.
	\end{equation*}Therefore, the right-hand side of \eqref{eqn:aGegen} converges uniformly in $[-1,1]$ because $\lambda-N<-1$.
