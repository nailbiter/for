...
we set\myfootnote{the results in the table below concern $\lambda\in \Azeven(p,q)$ instead of the more general $A_0(p,q)$. The classification can (I guess) by generalized to the latter, but
...
\begin{equation*}
	...
	A_0(p,q):=\left\{
		...
		\end{array}
		\right.\\
		\Azeven(p,q):=\left\{ \lambda\in A_0(p,q)\mid \lambda-\frac{p-q}{2}+1\in2\Z\right\}\subset A_0(p,q)\\
	\end{array}
\end{equation*}
Let $p>1$.
We recall from \cite{KO2} that
for any $\lambda\in A_0(p,q)$ each of the following 
\underline{5 conditions defines uniquely a $(\mathfrak{g},K)$-module,} \underline{which are mutually isomorphic}
\myfootnote{although this is a direct citation of \cite{KO2}, I have some problem understanding its English:
``a $(\mathfrak(g),K)$-module'' is in plural, while ``which are'' is in singular$\cdots$
}. We shall denote
it by $\left( \pi_{+,\lambda}^{p,q} \right)_K$. The $(\mathfrak{g},K)$-module $\left( \pi_{+,\lambda}^{p,q} \right)_K$ is non-zero and irreducible.
\begin{enumerate}
...
\end{enumerate}
\underline{Each of realization}\myfootnote{maybe, "Each of realizations"?}
...
From now, we set $\left( G,G' \right)=\left( O(p+1,q+1),O(p,q+1) \right)$\myfootnote{we have defined $G$ as $G=O(p,q)$ above. For not very careful
readers, this might be confusing}.
...
%In fact, by  to conditions In fact, conditions (i) and (i)$'$ make sense for a bigger parameter sets
\begin{definition}[$Y_{\pm,\lambda}^{p,q}$]
	Let\myfootnote{I will complete this definition in next version}
	$p,q\in\N_+$. We define the parameter set $\Bzeven$ as
\begin{equation*}
	\kern-0cm\begin{array}[]{c} 
	\Azeven(p,q)\subset \Bzeven(p,q):=\left\{
		\begin{array}[]{@{}ll} 
			%\emptyset,			&p=1,q>1,\\
			%A_0(p,q),			&p\ge q=1,\\
			\frac{2-p-q}{2}+2\N_+,		&p\in2\N_+,q\in2\N_++1,\\
			\frac{p-q}{2}+2\Z+1,		&p\in2\N_++1,q\in2\N_+,\\
...
for any $\lambda\in \Bzeven(p,q)$ each of the following conditions defines uniquely a
$(\mathfrak{g},K)$-module, which are mutually isomorphic. We shall denote
it by $\left( Y_{+,\lambda}^{p,q} \right)_K$. The $(\mathfrak{g},K)$-module $\left( Y_{+,\lambda}^{p,q} \right)_K$ is non-zero and irreducible.
\myfootnote{by considering submodules/quotients of $I(\lambda\otimes\varepsilon)$ for $\varepsilon=\pm1$ we can, perhaps, even enlarge $
\Bzeven(p,q)$. Should I mention this?}
\begin{enumerate}
	\item[(i)] A subrepresentation of
		the degenerate principal series representation
		$\Ind_{P^{\scalebox{0.5}{$\max$}}}^G\left( \mathbb{C}_{\lambda} \right)$
		with $K$-type\begin{equation*}
			\begin{array}[]{c}
				\Xi(K:b)\equiv \Xi\left( O(p)\times O(q):b \right)\\:=\displaystyle
				\bigoplus_{\scalebox{0.5}{$\begin{array}[]{c}
			...
			\end{array}
		\end{equation*}
	\item[(i)$'$] A quotient of $\Ind_{P^{\scalebox{0.5}{$\max$}}}^G\left(\otimes \C_{-\lambda} \right)$ with $K$-type $\Xi(K:b)$.
\end{enumerate}
Each of realization (i), (i)$'$, (ii), and (iii) also gives a globalization of $\ppqpl$,
namely, a continuous representation of $G$ on a topological vector space. Because all of $(\ppqpl)_K\;\left( \lambda\in A_0(p,q) \right)$ are unitarizable we may and do take the globalization 
$\ppqpl$ to be the unitary representation of G.
...
\end{remark}
Now, the latter definition allows us to give the following generalization of Theorem \ref{thm:Aq}:
