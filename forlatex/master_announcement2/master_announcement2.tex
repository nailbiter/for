...
\myfootnote{I will define $B(p,q)$ and $Y_{\pm,\lambda}^{p,q}$ for $B(p,q)$ in next version}
\begin{enumerate}
...
(the latter corresponds to the $q=0$ case of the former\myfootnote{in fact, $q=0$ case is omitted from the Theorem \ref{thm:Aq}, so the word
``generalization'' might not be the most appropriate\ldots}).
\end{remark}
