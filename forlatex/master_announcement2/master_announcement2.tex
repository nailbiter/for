...
for any $\lambda\in A_0(p,q)$ each of the following conditions defines uniquely a $(\mathfrak{g},K)$-module, which are mutually isomorphic. We shall denote
it by $\left( Y_{+,\lambda}^{p,q} \right)_K$. The $(\mathfrak{g},K)$-module $\left( \pi_{+,\lambda}^{p,q} \right)_K$ is non-zero and irreducible.
\myfootnote{by considering submodules/quotients of $I(\lambda\otimes\varepsilon)$ for $\varepsilon=\pm1$ we can, perhaps, even enlarge $A_0'(p,q)$. Should I mention this?}
\begin{enumerate}
