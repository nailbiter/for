...
\end{remark}
\begin{remark}
	Note that the bottom-rightmost entry of every table jointly generalize \myfootnote{I think word ``generalize'' might not
	be entirely correct here, as
	\cite[Thm. 3.3]{kobayashi93restriction} is in fact stronger than the statement we can read off Theorem \ref{thm:Aq}:
	not only it shows with which multiplicitly $\pi_{-,y}^{p,q+1}$ occur in $\pi_{-,x}^{p+1,q+1}\mid_{G'}$, but it also
	shows that {\it only} these terms occur in restriction.
}
	\cite[Thm. 3.3]{kobayashi93restriction}.
\end{remark}

By analogy with $\Azeven(p,q)$ defined above, we define \begin{equation*}
	\Azeven(p,q)\subset \Bzeven(p,q):=\left\{ \lambda\in B(p,q)\left| \lambda-\frac{p-q}{2}+1\in2\Z \right.\right\}.
\end{equation*}
