...
\newcommand{\myrestr}[2]{\left.#1\right|_{#2}}
...
\newcommand{\mypif}[1]{\pi^{n+1,1}_{-,#1}}
\newcommand{\mypifp}[1]{\pi^{n,1}_{-,#1}}
		
%m\left( ,\pi^{n,1}_{+,\frac{n-1}{2}+j} \right)=1,\quad m\left(  \right)
...
\affil[1]{Kavli IPMU and Graduate School of Mathematical Sciences, The University of Tokyo}
\affil[2]{Graduate School of Mathematical Sciences, The University of Tokyo}
...
\textbf{Keywords:} {Representation theory}, {reductive group}, {branching law}, {broken symmetry}, {conformal geometry}, {symmetry breaking operator}, {Zuckerman modules}\\
\textbf{Subject}: 2010, {22E46; 33C45, 53C35}
\end{abstract}
\input{useless.tex}
...
%%here K insertion starts
\begin{equation*}
	\begin{array}[]{c}
		\tilde{\lambda}:=\lambda-\frac{p+q-2}{2}\\
		A_+^{p,q}(\tilde{\lambda}):=\pi_{+,\lambda}^{p,q}
	\end{array}
\end{equation*}
The the $\mathfrak{z}(\mathfrak{g})$-infinitesimal character of $A_+^{p,q}(\tilde{\lambda})$ is\begin{equation*}
	\left( \tilde{\lambda},0,\cdots,0 \right)+\left( \frac{p+q}{2}-1,\frac{p+q}{2}-2,\cdots,\frac{p+q}{2}-\mysbra{\frac{p+q}{2}} \right).
\end{equation*}
The condition $\lambda>-1$ and $\lambda\in\Z+\frac{p+q}{2}$ amounts to\begin{equation*}
	\tilde{\lambda}>-\frac{p+q}{2}\mbox{ and }\tilde{\lambda}\in\Z.
\end{equation*}<++>
%%here K insertion ends
...
%%		\pi^{n+1,1}_{-,-\frac{n}{2}-i}\simeq F(i).
%%	\end{equation*}
...
%%						\Hom_{G'}\left( \mypit{x}\mid_{G'},\mypifp{y} \right)=0,\\
%%						\Hom_{G'}\left( \mypif{x}\mid_{G'},\mypifp{y} \right)=1.
						\end{array}
					\end{equation*}
				\item [(1-b)] Assume $x-y+\frac{1}{2}\in2\Z$. Then
					\begin{equation*}
						\begin{array}[]{c}
						\Hom_{G'}\left( \myrestr{\mypit{x}}{G'},\mypitp{y} \right)=0,\\
%%						\Hom_{G'}\left( \mypit{x}\mid_{G'},\mypifp{y} \right)=1,\\
%%						\Hom_{G'}\left( \mypif{x}\mid_{G'},\mypifp{y} \right)=0.
						\end{array}
					\end{equation*}
			\end{enumerate}
		\item Suppose that $x<y+\frac{1}{2}$. Then
			\begin{equation*}
				\begin{array}[]{c}
				\Hom_{G'}\left( \myrestr{\mypit{x}}{G'},\mypitp{y} \right)=0,\\
%%				\Hom_{G'}\left( \mypit{x}\mid_{G'},\mypifp{y} \right)=1,\\
%%				\Hom_{G'}\left( \mypif{x}\mid_{G'},\mypifp{y} \right)=0.
			...
			\end{equation*}
	\end{enumerate}
	In this notation, the results of Theorem \ref{thm:Aq} can be seen as
	an extension of \cite[Thm. 1.2]{kobayashi2015program}, the latter corresponding to $p=n,q=0$ case.
%%	\myfootnote{by using notation $Y^{n+1,1}_{\pm,x}$ below,
%%	the results of the remaining items of \cite[Thm. 1.2]{kobayashi2015symmetry} can be seen as special cases of Theorem \ref{thm:Y} below. Should I include these?}
\end{remark}
\begin{remark}
...
%%	\cite[Thm. 3.3]{kobayashi93restriction}.
	...
	We then have
	\begin{equation*}
		\kern-1cm\myrestr{\pi^{q,p}_{-,\lambda}}{O(q-s)\times O(s,p)}\simeq \bigoplus_{a,k\in\N} \mathcal{H}^a\left( \R^{q-s} \right)\boxtimes\pi_{-,\lambda+\frac{1}{2}(q-s)+a+2k}^{s,p}.
	\end{equation*}
	In particular, taking $s=q-1$ we get
	\begin{equation*}
		\kern-1cm\myrestr{\pi^{q,p}_{-,\lambda}}{O(q-1,p)}\simeq \bigoplus_{k\in\N}\pi_{-,\lambda+\frac{1}{2}+2k}^{q-1,p}.
	\end{equation*}
	The latter, in particular, implies that for $(G,G')=\left( O(q,p),O(q-1,p) \right) we\;have
	$\begin{equation*}
		\begin{array}[]{c}
			\Hom_{G'}\left(\myrestr{\pi^{q,p}_{-,\lambda}}{G'}, \pi^{q-1,p}_{-,\nu} \right)=\left\{\begin{array}[]{ll}
	...
	More precisely, this follows from the last row
	(the one with $\pi_{-,x}^{p+1,q+1}$ in the leftmost cell) of every item's table.
\end{remark}
...
%%		with $K$-type\myfootnote{in one of the previous revisions (see bottom-right of first page of attached {\ttfamily revision.pdf}) you told me I ``cannot
%%		use the same letter''. That's why I've replaced $\Xi$ with $\Sigma$. But as far as I understand, there is no difference between them.}\begin{equation*}
%%			\begin{array}[]{c}
...
%%			\end{array}
%%		\end{equation*}
%%		with $b$ being the same as in the definition of $\Xi(K:b)$ above. Note that for $\lambda\in A_0(p,q)$ we have $\Xi(K:b)=\Sigma(K:b)$.
%%	\item[(i)$'$] A\myfootnote{I think, there should be no assumptions regarding the parity of $p,q$} quotient of $\Ind_{P^{\scalebox{0.5}{$\max$}}}^G\left( \varepsilon\otimes\C_{-\lambda} \right)$ with $K$-type $\Sigma(K:b)$.
%%\end{enumerate}
