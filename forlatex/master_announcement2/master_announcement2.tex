...
%%	\cite[Thm. 3.3]{kobayashi93restriction}.
	...
	We then have
	\begin{equation*}
		\kern-1cm\pi^{q,p}_{-,\lambda}\mid_{O(q-s)\times O(p,s)}\simeq \bigoplus_{a,k\in\N} \mathcal{H}^a\left( \R^{q-s} \right)\boxtimes\pi_{-,\lambda+\frac{1}{2}(q-s)+a+2k}^{s,p}.
	\end{equation*}
	In particular, taking $s=q-1$ we getting
	\begin{equation*}
		\kern-1cm\pi^{q,p}_{-,\lambda}\mid_{O(p,s)}\simeq \bigoplus_{k\in\N}\boxtimes\pi_{-,\lambda+\frac{1}{2}(q-s)+k}^{q-1,p}.
	\end{equation*}
\end{remark}
