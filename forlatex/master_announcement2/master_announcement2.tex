...
5 conditions defines uniquely $(\mathfrak{g},K)$-module, which are mutually isomorphic.
\myGrammarNoteFootnote{although this is a direct citation of \cite{KO2}, I have some problem understanding its English:
``a $(\mathfrak(g),K)$-module'' is in singular form, while ``which are'' is in plural\dots
} We shall denote
...
\end{enumerate}
Zuckeran's derived fuctor modules $\mathscr{R}_{\mathfrak{q}}^S\left(\mathbb C_\lambda\right)$
are defined as an algebraic analogue of the geometric
quantization of elliptic coajoint orbits,
which gives a vast generalization of Borel-Weil-Bott theorem
for comacct Lie groups.
...
\begin{equation*}
\kern-1cm\Azeven(p,q):=\left\{ \lambda\in A_0(p,q)\mid \lambda-\frac{p-q}{2}+1\in2\Z\right\}\subset A_0(p,q)\\
\end{equation*}
...
%%		with $K$-type
		\begin{equation*}
			...
			\end{array}
		\end{equation*}
		We define the following subsets of $\C^2$\myfootnote{in fact, this notation is redundant: it does not appear in subsequent}:
\begin{equation*}
	\begin{array}[]{c}
		//:=\left\{ (\lambda,\nu)\in\C^2\mid \lambda-\nu\in-2\N \right\},\\
...
\begin{definition}[$Y_{\pm,\lambda}^{p,q}$]\label{def:Y}
	Let $p,q\in\N_+$. We define the parameter set $\Bzeven$ as
\begin{equation*}
...
Kobayashi, Toshiyuki. Singular unitary representations and discrete series for indefinite Stiefel manifolds U (p, q; F)/U (pm, q; F). No. 462. American Mathematical Soc., 1992.
...
Vogan, David A. “Unitarizability of Certain Series of Representations.” Annals of Mathematics, vol. 120, no. 1, 1984, pp. 141–187. Second Series, www.jstor.org/stable/2007074.
