...
\newcommand{\mypif}[1]{\pi^{n+1,1}_{-,-\frac{n}{2}-#1}}
...
%%						\Hom_{G'}\left( \mypit{i}\mid_{G'},\mypifp{j} \right)=0,\\
%%						\Hom_{G'}\left( \mypif{i}\mid_{G'},\mypifp{j} \right)=1.
						\end{array}
					\end{equation*}
				\item [(1-b)] Assume $i\equiv j\mbox{ mod 2}$. Then
					\begin{equation*}
						\begin{array}[]{c}
						\Hom_{G'}\left( \mypit{i}\mid_{G'},\mypitp{j} \right)=0,\\
%%						\Hom_{G'}\left( \mypit{i}\mid_{G'},\mypifp{j} \right)=1,\\
%%						\Hom_{G'}\left( \mypif{i}\mid_{G'},\mypifp{j} \right)=0.
						\end{array}
					\end{equation*}
			\end{enumerate}
		\item Suppose that $i<j$. Then
			\begin{equation*}
				\begin{array}[]{c}
				\Hom_{G'}\left( \mypit{i}\mid_{G'},\mypitp{j} \right)=0,\\
%%				\Hom_{G'}\left( \mypit{i}\mid_{G'},\mypifp{j} \right)=1,\\
...
%%	\cite[Thm. 3.3]{kobayashi93restriction}.
	In \cite{kobayashi93restriction} non-zero irreducible $\left( \mathfrak{o}(p,q),SO(p)\times SO(q) \right)$-modules $U_{\pm}^{SO_0(p,q)}(\lambda)$ we introduced.
	Comparing notation of \cite{kobayashi93restriction} with that of this announcement, we see that\begin{equation*}
		U_{\pm}^{SO_0(p,q)}(\lambda)\simeq \pi^{p,q}_{\pm,\lambda}\mid_{SO_0(p,q)}.
	\end{equation*}
	Using this identification, we can rewrite the results of \cite[Thm. 3.3]{kobayashi93restriction} as follows:
\end{remark}
