...
%%	\cite[Thm. 3.3]{kobayashi93restriction}.
	...
	\begin{equation*}
		\kern-1cm\pi^{q,p}_{-,\lambda}\mid_{O(q-1,p)}\simeq \bigoplus_{k\in\N}\pi_{-,\lambda+\frac{1}{2}+k}^{q-1,p}.
	\end{equation*}
	The latter, in particular, implies that for $(G,G')=\left( O(q,p),O(q-1,p) \right) we have$\begin{equation*}
		\begin{array}[]{c}
			\Hom_{G'}\left(\pi^{q,p}_{-,\lambda}\mid_{G'}, \pi^{q,p}_{-,\nu} \right)=\left\{\begin{array}[]{ll}
				1,&\nu-\lambda-\frac{1}{2}\in\N\\
				0,&\mbox{otherwise,}
			\end{array}\right.\\
			\Hom_{G'}\left(\pi^{q,p}_{-,\lambda}\mid_{G'}, \pi^{q,p}_{+,\nu} \right)=0.
		\end{array}
