...
In fact, conditions (i) and (i)$'$ make sense for a bigger parameter sets\begin{equation*}
	\kern-2cm\begin{array}[]{c} 
	A_0(p,q)\subset A_+'(p,q):=\left\{
		\begin{array}[]{ll} 
			\emptyset,&p=1,q>1\\
...
defined for $\lambda\in A_+'(p,q)$ (or $\lambda\in A_-'(p,q)$ for $Y_{-,\lambda}^{p,q}$).
