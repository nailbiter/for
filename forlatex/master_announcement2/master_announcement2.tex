...
we set\myfootnote{the results in the table below concern $\lambda\in A^{\mbox{\normalfont \footnotesize even}}_0(p,q)$ instead of the more general $A_0(p,q)$. The classification can (I guess) by generalized to the latter, but
...
\begin{equation*}
	...
	A_0(p,q):=\left\{
		...
		\end{array}
		\right.\\
		A^{\mbox{\normalfont \footnotesize even}}_0(p,q):=\left\{ \lambda\in A_0(p,q)\mid \lambda-\frac{p-q}{2}+1\in2\Z\right\}\subset A_0(p,q)\\
	\end{array}
\end{equation*}
Let $p>1$.
...
In fact, conditions (i) and (i)$'$ make sense for a bigger parameter sets
...
\begin{theorem}[$G'$-invariant maps between $Y_{\pm,\lambda}^{p+1,q+1}$]\label{thm:Aq}
