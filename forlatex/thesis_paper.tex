...
The system 
\[x'(t)=A(t)x(t)\]
where $x(t)$ and $A(t)$ are respectively vector-valued and square-matrix-valued function defined for $t\in S\subseteq\mathbb{R}$.
...
$E^+\oplus E^-$ and for arbitrary $x^+\in E^+,\; x^-\in E^-$ and $a,b\in S$ we have the following:
...
for some constants $K,\;\gamma>0$, that do not depend on $x^+,x^-$
...
give up this assumption, but for simplicity we shall use it. In particular, continuity assumption will let us immediately assume
