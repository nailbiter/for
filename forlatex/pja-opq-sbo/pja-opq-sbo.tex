...
\newcommand{\mypicwtitle}[3]{\begin{figure}[H]\centering\includegraphics[width=#1\textwidth]{#2}\\{#3}\end{figure}}
\newcommand{\myexplain}[1]{\mbox{ (\textit{#1})}}
\newcommand{\myr}[1]{\mathbb{R}^{#1}}
\newcommand{\mys}[1]{\mathbb{S}^{#1}}
\newcommand{\mybramatii}[4]{\left(\begin{array}{rr}#1&#2\\#3&#4\end{array}\right)}
\newcommand{\mybramatiii}[9]{\left(\begin{array}{lll}#1&#2&#3\\#4&#5&#6\\#7&#8&#9\end{array}\right)}
...
%custom arrow to denote diff-sm
\newcommand{\diffsm}{\xrightarrow{\sim}}

%more conventional phi
\let\oldphi\phi
\let\phi\varphi
...
%environments for proofs, lemmas, theorems etc.
\theoremstyle{plain}
...
\newtheorem{myprob}[mythm]{Problem}
...
in a separate paper.
% together with an application to the restriction problem of Zuckerman derived functor modules.
...
find a distribution $K_T\in\mathcal{D}'\left( \R^n \right)$ such that for all $f\in I(\lambda)$
\begin{equation*}
\iota^*_\nu(Tf)(x')=
...
\begin{enumerate}[(1)]
    ...
    \item $F$ is invariant by $N_+':=N_+\cap G'$.
  \end{enumerate}
\end{definition}
Applying the general results proven in \cite[Chap.\ 3]{kobayashi2015symmetry} to our particular setting, we get the following:
...
\end{fact}
For $T\in\SBO$, 
a closed $P'$-invariant subset $\Supp(T)$ in $X=G/P$
...
By \cite[Lem.\ 2.22]{kobayashi2016differential1},
...
%We do not need SBOs associated to $S=C\cap Y$ for $p=1$;
...
\bibitem[13]{kobayashi2015branching}
...
\bibitem[14]{kobayashi2013finite}
...
\bibitem[15]{kobayashi2016differential1}
...
\bibitem[16]{kobayashi2015symmetry}
