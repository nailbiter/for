...
\begin{definition} \label{def1}
	Let $h(b,x):=1-2\,^t\!bI_{p,q}x+\Q(b)\Q(x)$ for $b,x\in\R^{n}\;(n=p+q)$\footnote{I think we might use $c(b)$ in place of $h(b,x)$ in order
		to keep up with the notation of \cite[p. 47]{kobayashi2015symmetry}}. A distribution
...
$X^{p,q}:=\Xi^{p+1,q+1}/\R^{\times}\simeq G/P$. We set
\[
...
\begin{theorem}[classification of closed $P'$-invariant subsets of $G/P$]
  ...
  \begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.3\textwidth}
	    \xymatrixrowsep{0.5pc}
	    \xymatrix{&X\ar@{-}[ld]_1\ar@{-}[rd]^1&\\Y\ar@{-}[rd]_1&&C\ar@{-}[ld]^1\\&C\cap Y\ar@{-}[dd]^{p+q-2}&\\&&\\&\{[0]\}&}
	\caption{when $p>1$}
    \end{subfigure}
...
\begin{remark}
	By definition, $R_{\lambda,\nu}^{ \left\{ 0 \right\}}$ in Theorem \ref{thm:construction} amounts to
	\begin{equation*}
		R_{\lambda,\nu}^{ \left\{ 0 \right\}}=
	...
	\end{equation*}
	This formula for differential SBO was previously found in \cite[Thm. 5.2.1, Thm. 5.2.1]{juhl2009families}, \cite[(10.1)]{kobayashi2015symmetry} for $q=0$ and in \cite[Thm.\ 4.3]{kobayashi2015branching}
	for general $p,q$.
\end{remark}
...
\end{remark}
We can find basis for $\SBO$ for every $(\lambda,\nu)\in \mathbb{C}^2$ as follows:
...
\end{corollary}
The degenerate principal series representation $I(\lambda)$ of $G$ contains one-dimensional subspace of spherical vectors (i.e. $K$-fixed vectors), and likewise $J(\nu)$ of $G'$.
...
\end{theorem}
\begin{remark}
	Theorem \ref{thm:spherical} was known in \cite[Lemma A.5]{bernstein2004estimates}, \cite[Thm. 1.1]{clerc2011generalized} for $p=q=1$ and in \cite[Prop.\ 7.4]{kobayashi2015symmetry} for $q=0$.
\end{remark}
...
\begin{theorem}[images of SBOs]
	...
	for $p>1$:
\end{theorem}
		\newgeometry{left=0.7cm,top=1cm,bottom=2cm}
	\begin{enumerate}[(1)]
