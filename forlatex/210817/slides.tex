...
%%次に第$p$座標の固定部分群を
%%\begin{equation*}
%%	G':=\mysetn{g \in G}{g \cdot e_{p + 1} = e_{p + 1}}\subset G\quad\mbox{とする。}
%%\end{equation*}
%%\begin{equation*}
...
%%	{\item {$P'$のラングランズ分解は$P'=(G'\cap M)A(G'\cap N_+)$で与えられる。}}
%%\end{enumerate}
%%$I(\lambda)$と同様
...
対称性破れ作用素全体のなす空間\footnote{
I understood Your instruction ``その次に現在のpage 5と６を最初のスライドとする。'' in the sense that I should
remove slides 3 and 4, and so I did. Please, note that this removes the definition of $I(\lambda)$.}を$$\sbo\mbox{と表記す{る}。}$$
\end{block}
\note{
この講演での主テーマは{、}
...
以下では\begin{equation*}
	g(t):=\left\{ \begin{array}[]{ll}
		...
		0&\left(t\nin \left(2\N+\frac{1}{2}  \right)  \right)
