...
具体的に言うと、重要な問題
を挙げましょう:まずすべての複素数パラメーター $(\lambda,\nu)$に対して、対称性破れ作用素の空間の次元を
決定したいです。これを出来れば、もっと詳しくて、規定を具体的に作りたいです。
最後は、規定を具体的にわかれば、対称性破れ作用素の函数等式などの性質を具体的に求めたいです。

私の知識として、ノンコンパクト群の設定の上で、この質問の完全な答えを得た最初の例は、小林先生とSpeh先生の２０１５年に出版された本です。
今の言葉で言うと、あの本で$q=0$の場合を扱いました。従って、今の結果ははKobayashi--Spehの一般化として見なせます。
}
\end{frame}
...
最後のべき零リー群不変性についてちょっとだけご注意したいですが、
...
%%簡単のため、$A_0^{\mbox{\scriptsize even}}(p,q):=\left\{ \lambda\in A_0(p,q)\mid \lambda-\frac{p-q}{2}+1\in2\Z \right\}$とおく。
