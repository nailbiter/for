...
\begin{frame}\titlepage
	...
	今回の第５６回実函数論・函数解析学合同シンポジウム
	での講演にお\kana{招}{マ}ねき頂き
	、誠にありがとうございます。
...
\note{\mytiming{qform}
次に、扱う群の設定について説明したいと思います。

	$n=p+q$と{し}、$\R^n$上の符号$(p,q)$をもつ標準二次形式$Q_{p,q}$を定めます。
	標準座標でこの二次形式は\kana{対角行列}{タイカギョウレツ}$I_{p,q}$で表されます。
	二次形式$Q_{p,q}(x)$を保つ不定値直交群を$O(p,q)$と\kana{表記}{ヒョウキ}します。

	以下では、次元を１つずつ上げて得られる不定値直交群$O(p+1,q+1)$を$G$と表記します。
}
\end{frame}
...
これでもうまく出来た上で、基底を具体的にわかれば、対称性破れ作用素の函数等式などの性質を具体的に求めたいです。
...
\tiny
		Kobayashi--Spehの戦略に\kana{倣って}{ナラッテ}、
		\kana{第一歩}{ダイイッポ}として、対称性破れ作用素を、
		超関数核で
		記述し、その性質を調べます。超函数核の存在はSchwartzの核定理にもとで分かるものです。
		Kobayashi-Spehの本で証明された一般理論を今の特別な設定に\kana{適
		用}{てきよう}すると、
		以下の図式を得ます。

		下の$S\mbox{ol}\left(\mathbb{R}^{p,q}
		\right)$という空間はある$(\lambda,\nu)$に依存する偏微分方程式を満たす超関数空間です。ここで、左から真ん中への同型写像はSchwartzの\kana{核}{カク}
		定理を\kana{用}{モチ}います、真ん中から下へのrestという写像は、open Bruhat cell
		と呼ばれる\kana{座標近傍}{ザヒョウキンボウ}
		に\kana{積分核}{セキブンカク}
		を制限することによって\kana{導}{ミチビ}かれます。この２つの写像は線形空間の同型写像になります。
		このようにして、抽象的な対称性破れ作用素空間の代わりに具体的なEuclid空間上の\kana{連立}{レンリツ}偏微分方程式の解空間を研究に\kana{置}{オ}き\kana{換}{カ}えることができます。
		ポイントはもう１つあります。真ん中の空間は$G/P$上の$P'$不変超関数空間なので、それぞれの元のサポートが$P'$不変$G/P$閉部分集合になります。
		つまり、真ん中のベクトル空間から$G$の\kana{両側剰余}{リョウガワジョウヨ}空間へのが\kana{定}{サダ}{まった}ということです。Target spaceであるこの\kana{両側剰余}
		{リョウガワジョウヨ}空間は有限集合なので、$P'$不変$G/P$閉部分集合が大切なinvariantになります。
	}
\end{frame}
...
\kana{直前}{チョクゼン}のスライドにおける
Factで$sol$と言う記号を書きました。この$\sol$と言う
記号を説明したいと思います。
$\sol$と言うのは、以下の４つ条件を満たす超関数のなす空間でございます。
それぞれの条件は実際には偏微分方程式の形で述べられます。
...
\begin{frame}{実旗多様体の軌道分解$P'\backslash G/P$}
	...
	\end{theorem}
	\note{\mytiming{setinv}
	$P'$不変 $G/P$ 閉部分集合が対称性破れ作用素の積分核の大切な不変量になるので、次に
、両側\kana{剰余類}{ジョウヨルイ}とその\slowly{closure relations}、\kana{閉包}{ヘイホウ}関係の\kana{記述}{キジュツ}を述べます。
...
\begin{frame}{対称性破れ作用素の像}
	\begin{block}{問題}
		全ての$(\lambda, \nu) \in \mathbbm{C}^2$に対し、上記で構成されたそれぞれの対称性破れ作用素\begin{equation*}
			I(\lambda)\to J(\nu)
		\end{equation*}の像を具体的に決定せよ。
	\end{block}
	\begin{theorem}
	...
	\note{\mytiming{images}
	まだ述べていなかった重要な問題として、
	対称性破れ作用素の像の決定問題を取り挙げてみましよう。
	すなわち、
	全ての$(\lambda, \nu) \in \mathbbm{C}^2$に対し、上記で構成されたそれぞれの対称性破れ作用素\begin{equation*}
		I(\lambda)\to J(\nu)
	\end{equation*}の像を具体的に決定しましょう。
	
	この答えは$\lambda,\nu$によって変化します。ここでは詳しく述べませんが、
	上記の問題も完全に\kana{解決}{カイケツ}することができました。
	アブストラクト(page ??)
	にはこれを図式表示いたしました。
