...
\begin{frame}{扱う群の設定 -- 不定値直交群}
	  ...
	  $。
	  この二次形式$Q_{p,q}(x)$を保つ\footnote[frame]{I think, we should write 二次形式$Q_{p+1,q+1}(x)$を保つ, as $O(p+1,q+1)$ preserves $Q_{p+1,q+1}$, not $Q_{p,q}$.}不定値直交群を
\begin{equation*}
	G \assign O (p +
...
$G$の球退化主系列表現$I(\lambda)$Яを定義する。\footnote[frame]{I think this slides violates somehow the "1 slide 1 topic" policy:
