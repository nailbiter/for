...
\begin{equation*}
	\quad| x_p |^{\lambda + \nu - n} | Q_{p, q} |^{- \nu}\mbox{
		は$L^1_{\operatorname{loc}}\left( \R^{p+q} \right)$\footnote[frame]{I think it is continuous, not only $L^1_{\operatorname{loc}}$}であり、
...
\begin{frame}{Knapp--Stein 作用素}
	\begin{definition}
		...
		\end{equation*}
		ここで
		$q (\lambda)$ は$\Gamma$関数の積に\doubt{よる}正規化のための関数
	\end{definition}
...
\begin{frame}{函数等式の考え方}
	...
	\begin{block}{Observation}
		合成$\color{red}{\uparrow}\color{black}{\searrow}$や$\searrow\color{red}{\uparrow}$なども対称性破れ作用素となる。
	\end{block}
	\vspace{-1em}
