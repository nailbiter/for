...
Research (A) (25247006), Japan Society for the Promotion of Science.} (東京大学\;大学院数理科学研究科{・}\kern3.75cm\\カブリ数物連携宇宙研究機構)\\
...
極大放物型部分群
$P=MAN_{+}$を以下のように定める、
\input{master_extract_tagsetting}
...
\begin{theorem}[対称性破れ作用素の分類]\label{thm:classif}
	$p,q\ge1$とする。
	\input{master_extract_classification}
\end{theorem}
...
\begin{theorem}[留数公式]
  ...
  \right), & \min\left\{ p,q \right\}>0,\frac{n}{2} + p \in 2\mathbbm{Z}
	...
	このとき、
	\input{master_extract_functional}
...
\begin{theorem}[対称性破れ作用素の像]
	...
	\begin{enumerate}[{\rm (1)}]
		\item 
	$p>1$の場合:\\
	\input{p_bigger_than_one_image_diagram_explanation}
	...
	\item $p\tisoddjap,q\tisevenjap$とする。 このとき、$\nu\in2\Z,0<\nu<n-1$ならば、$R_{\lambda,\nu}^X$ は全射である。
	  ...
	  \end{figure}
		\begin{figure}[H]
			\noindent\begin{tabular}{m{1.3cm}rrr}
	      $(\lambda,\nu)\in$&$\mybra{//\cup\backslash\backslash}^c$ & $//-\backslash\backslash$  & $//\cap\backslash\backslash,k< l$\\[0pt]
	      \vspace{-3cm}$\begin{array}{l}\nu\todd\\\nu\ge\frac{n+1}{2}\end{array}$&\input{f5}\\
	    \end{tabular}
	  \end{figure}
			\newpage
...
\item
	$p=1$の場合。\\
	\input{p_equal_to_one_image_diagram_explanation}
	\end{enumerate}
\end{theorem}
