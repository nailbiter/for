...
\begin{frame}{Why Hermite Networks}
	...
	\begin{itemize}
		\item $h_0$ is Gaussian filter\begin{equation*}
				\implies e^{-t^2/2}\ast y\approx e^{-t^2} \ast y =\sum_{i=1}^N \hat{\omega}_i \left( h_i\ast h_0 \right)
			\end{equation*}
			\pause
		\item Invariant under Fourier transform: $\mathcal{F}(h_n)=\left( -i \right)^n\sqrt{2\pi}h_n$;
	\end{itemize}
\end{frame}
