...
\begin{frame}{Advantages}
	\begin{itemize}
		...
		\item Many strategies for the training of coefficients; \pause
...
\begin{frame}{Why Hermite Networks?}
	...
	\begin{itemize}
		...
		\item Can be implemented in hardware \cite{osowski1992neural,cochocki1993neural,chesnokov1999new};\pause
	\end{itemize}
...
\begin{frame}{But we can do more\dots}
	...
	\pause
	\begin{enumerate}
		\item Define {\it ``Gegenbauer'' Neural Networks} and give an explicit formula for convolution for them;\pause
		\item Compare them with other common types of networks for signal reconstruction (as in \cite{mackenzie2003hermite});\pause
		\item Find signals ($\implies$ classes of problems) for which GNN will be advantageous;\pause
		\item As Hermite functions can be applied directly to image processing \cite{martens1990hermite,venkatesh1994hermite},
			we can try to find direct application of $\eqref{eqn:5}$ to signal processing as well.
	\end{enumerate}
\end{frame}
