...
\begin{frame}
	...
	\begin{equation}
		...
		a_{\lambda, \mu, \nu}^{\ell, m} C_{\ell}^{\lambda} (s) C_m^{\mu} (t),\quad a^{\ell,m}_{\lambda,\mu,\nu}=\dots,
		\label{eqn:4}
	\end{equation}
	\pause
	\begin{equation}
...
\begin{frame}
	...
	\begin{equation*}
		\begin{array}{llll}
			&\int_{-\infty}^\infty\myabs{w}^{2\nu}&\int_{-\infty}^\infty hr_m\left( s-w \right)hr_n(s)\mbox{ds\;dw}&=\dots\\
			(\mbox{Mellin transform }\rightarrow)&\mathcal{M}( &hr_m\ast hr_n )&=\dots
		\end{array}
	\end{equation*}\pause\begin{equation}
		\mathcal{M}\left( hr_m\ast hr_n \right)=\dots
		\label{eqn:6}
	\end{equation}
	\pause Using inverse Mellin transform (and a bit of magic) we can recover {\it convolution formula for Hermite-Rodriguez functions}!
\end{frame}
