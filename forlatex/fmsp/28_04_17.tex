...
\begin{frame}{Why Convolution?}
	We often need to convolve two Hermite Neural Networks (e.g. to recover noised signal):
	\begin{equation*}
		y_1\ast y_2\approx \hat{y}_1\ast \hat{y}_2=\sum_{i,j=1}^N a^{(1)}_i a^{\left( 2 \right)}_j \left(h_i\ast h_j  \right)
	\end{equation*}
	\pause
	It is good that we have explicit formula:
	\begin{equation*}
		\left( h_n\ast h_m \right)(t)=\left\{\begin{array}[]{ll}
			\left( -1 \right)^{n+m}l_m^{n-m}\left( \frac{t^2}{2} \right),&t\ge0\\
			l_{m}^{n-m}\left( \frac{t^2}{2} \right),&t<0
		\end{array}\right.(m\le n).
	\end{equation*}
\end{frame}
