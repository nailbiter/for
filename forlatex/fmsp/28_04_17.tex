...
\begin{frame}
	...
	\begin{equation}
		\begin{array}{c}
			\int_{-\infty}^\infty\int_{-\infty}^\infty \myabs{w}^{2 \nu} c_m^{\mu} (s - w) \;
...
\begin{frame}{But we can do more\dots}
	...
	\pause
	\begin{enumerate}
		\item Define {\it ``Gegenbauer'' Neural Networks} and compute correlation for them;
		\item Compare them with other common types of networks for signal reconstruction (as in \cite{mackenzie2003hermite})
	\end{enumerate}
\end{frame}
