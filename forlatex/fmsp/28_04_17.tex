...
\begin{frame}{Why Hermite Networks}
	...
	\begin{itemize}
		\item $h_0$ is Gaussian filter\vspace{-0.5cm}\begin{equation*}
				\implies e^{-t^2/2}\ast y\approx e^{-t^2/2} \ast y =\sum_{i=1}^N \hat{\omega}_i \left( h_i\ast h_0 \right)
			\end{equation*}
			\vspace{-0.5cm}
			\pause
		\item Invariant under Fourier transform: $\mathcal{F}(h_n)=\left( -i \right)^n\sqrt{2\pi}h_n$ ($\implies
			\mathcal{F}\left( \hat{y} \right)=\sum_{j=1}^N \hat{\omega}_j (-i)^j \sqrt{2\pi}h_j$);
	\end{itemize}
\end{frame}
