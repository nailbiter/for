...
\newcommand{\solXi}{\sol[\Xi]\cap C(\Xi)}
\maketitle
...
We also let $M':=M\cap G'$, $N_+':=N_+\cap G'$ and $A':=A\cap G'=A$.
...
\begin{mydef}
	Having $(\lambda,\nu)\in\C^2$ and given $f\in C(\Xi)$ we say that $f\in\solXi$
...
\begin{mylem}
	Let $f\in \solXi$. Then it's pullback under $R^{p,q}\ni(u,v)\mapsto
	(1-\myabs{ u }^2+\myabs{ v }^2,2u,2v,1+\myabs{ u }^2-\myabs{ v }^2)\in\Xi$ gives a continuous function on $\R^{p,q}$
	which when seen as a generalized function, belongs to $\sol$.
\end{mylem}
\begin{myproof}
	As we are talking about the generalized functions defined by continuous functions,
	it suffices to show that pullback $F\in C(\R^{p,q})$
	of $f\in C(\Xi)$
	\begin{enumerate}
		\item $F(x)=F(-x)$;
	...
	should be satisfied by $f\in\solXi$.
\end{myproof}
\begin{myproof}(of {\bf Proposition 2.1})\newline
	...
	under the $R^{p,q}\ni(u,v)\mapsto
	[1-\myabs{ u }^2+\myabs{ v }^2,2u,2v,1+\myabs{ u }^2-\myabs{ v }^2]\in\Xi$ gives a continuous function. Hence, the previous
