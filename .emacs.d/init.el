...
;; PREPEND your custom paths to the system path so we don't lose /usr/bin/as
...
(custom-set-variables
 ...
 '(prettier-js-command "/opt/homebrew/bin/prettier")
...
;;; --- 6. LANGUAGES & BINARIES (ARM64 PATHS) ---
...
(defun pbcopy ()
  "Copy the entire buffer content to the macOS clipboard."
  (interactive)
...
(defun gemini ()
  "Run a Python script with current buffer content as stdin."
  (interactive)
  (let* ((user-prompt (read-string "Enter Prompt: "))
    ...
    (when (get-buffer output-buffer-name) (kill-buffer output-buffer-name))
...
;;; --- 8. LOCAL OVERRIDES & FINAL SETTINGS ---
...
;; Stop the async native-comp warning buffer from popping up
(setq native-comp-async-report-warnings-errors 'silent)

;; Or, if you want to see errors but ignore these linting warnings:
